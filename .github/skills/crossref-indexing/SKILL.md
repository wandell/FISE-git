...
1. **Enumerate LIVE candidate targets before writing anything.** A plain
   ...
   and figures are often commented out while chapters are being reorganized.
   A commented-out label will never resolve, no matter how correctly it's
   spelled. Filter comments out
...
2. **Map each concept to its best target.** Prefer the figure whose caption
   ...
   For a whole-topic pointer (not one figure), use the section's existing
   `{#sec-...}` label; if the section has none, fall back to a representative
   `@fig-` inside that section rather than inventing a chapter- or
   section-level label.
...
3. **Insert refs inline**, usually parenthetical, matching existing prose
   ...
   ```md
   * Photon-count variability (@fig-sensor-poisson)
   ```

4. **Verify every label before and after inserting it:**

   ```sh
   rg -n '#fig-sensor-poisson' chapters/
   ```
   ...
   frequently diverge (e.g. `images/sensors/13-photons/03-Poisson.png` is
   labeled `#fig-sensor-poisson`).
...
5. Only after ruling out 1–4: a stale `.quarto/xref` / `.quarto/idx` cache.
   Confirm and repair it with the procedure in `bib-crossref-indexing`
   ("Diagnose broken references"), which requires explicit authorization
   before deleting `.quarto/`. Do not delete `_book/`.
...
report it to the user with the exact file and warning text rather than
silently working around it.

## Avoid

- Inventing `@sec-`, `@fig-`, or `@eq-` labels that aren't verified to exist.
- Linking chapters with bare markdown paths (`](sensors-01-photoelectric.qmd)`)
