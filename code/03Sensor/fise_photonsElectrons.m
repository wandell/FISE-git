...
% We do not have to do the calculation in two steps. The calculation is the 
% same distribution if we combine the two probabilities. 
...
%% Counts
% These should be close to the Poisson distribution, which is close to the Normal 
% except when the Normal goes negative.
...
grid on;
%% 
% The mean and variance are equal in the Poisson.  So these should be close.  
% The problem with the Normal is that it predicts some negative values.  The Poisson 
% does not.  Everything is positive, so for small means they differ.
