...
In many applications we want to measure light from an extended, flat surface - such as a wall, the surface of a table, or even a large screen. To normalize an extended surface measurement, we make two corrections.  

First, we divide the energy (watts/nm) by the area of the surface as seen from the detector. For example, suppose the surface element we measure is a square $0.1$ meters on a side.  Then the surface area is $A_s = (0.1)^2~m^2$. Next, suppose the angle between the direction from the detector to the surface and the surface normal is $\theta$.  In that case, the foreshortened surface area is $A_s \cos(\theta) m^2$. 

The second geometric correction accounts for the three-dimensional angle of the cone of rays from the surface to the detector $\omega sr$. (This angle is not shown in @fig-radiometery.) This measurement, called the *radiance* $L$, has units of watts per nanometer per unit foreshortened area per steradian 

$$ L = \frac{Watts}{nm ~m^2 ~ sr} $$
...
A guide to help you remember the four fundamental radiometric measurements.  We will list the photometric measurements in @sec-human.
...
The difference between radiant intensity and radiance is a question of whether we are measuring from a point or a surface.  But as a surface patch becomes smaller and smaller, how do we know when it should be considered a point?  In many real-world situations, the distinction between a surface and a point source is not always clear-cut. It often requires careful consideration of the specific circumstances and the desired level of accuracy.Here are a few tips.

(Check with perplexity and chatgpt.  This from Gemini.)
...
* High Accuracy: If high accuracy is required, it's generally better to use the radiance formula, even for small surfaces. This allows you to account for the angular distribution of the emitted radiation.
* Lower Accuracy: If lower accuracy is sufficient, and the source can be reasonably approximated as a point source, the radiant intensity formula can be used.
:::
