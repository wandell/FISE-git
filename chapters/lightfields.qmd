...
Why do we sense and perceive the environment around us? For biology, the explanation is that animals rely on sensing to guide movements for food and to avoid predators. The ambient electromagnetic radiation is the best available source of information. Immobile life forms and machines that do not move or reach, such as trees and plants, use this radiation as a source of energy; but mobile life forms - a hawk, a rabbit, or a person - encode and interpret the radiation to glean information and interact with the environment. A main motivation for sensing in robotics, too, is to enable movement.  We endow robots and cars with image systems to enable them to identify and grasp objects, or to navigate from place to place.
...
Not all electromagnetic radiation is equally informative. High energy radiation (e.g., X-ray and Gamma ray bands) passes through most objects undisturbed. Low frequency radiation provides limited spatial resolution. Absorption and scattering of electromagnetic radiation in the wavelength range of a few hundred nanometers is useful for informing us about the environment because it is heavily absorbed and reflected by objects of interest. Specifically, the human eye detects electromagnetic radiation in the wavelength range from about 380 nm to 770 nm. We call this waveband "light", which is defined as "electromagnetic radiation that can be detected by the human eye."  Many image systems are designed to measure light; some measure electromagnetic radiation at wavelengths we do not see, thus extending our senses[^lightfields-1]. 
...
The rays of the incident light field can be described with one fewer parameter than the environmental light field because the entrance pupil is just a two-dimensional surface. Thus, the three dimensional position is replaced by the two-dimensional position in the entrance pupil, $(u,v)$.  There is, however, an additional set of implicit parameters that are needed to define the sensing system.  The aperture has a position in space $p$ and and a viewing direction $n$.  We 

$$L_I(u,v,\alpha,\beta,\lambda,\rho; \mathbold{p},(\mathbold{n}))$$
