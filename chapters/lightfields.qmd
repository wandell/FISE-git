...
Radiometry has standards for units that depend on the geometry of the measurement. Light field measurements are divided based on whether we are measuring a light field that is radiating from a source (radiance), or we are measuring a light field that is arriving at a surface (irradiance). They are further divided depending on whether we are measuring from a small (point) source, or from an extended surface.
...
@fig-radiometry illustrates the critical geometric differences between several radiometric measures. One important distinction is whether we are considering the light emitted from a location (radiance) or light arriving to a location (irradiance).
