...
Measuring the light field data is the start of seeing. The next step in seeing is to use use the data to find the things, stuff, material properties, and lights in the scene. This is the broad project of making computers that see.  

One important approach is build an internal model within the computer of different things, stuff, materials, and lights and calculate what the light field would be for this model. We compare the predicted light field with the measured light field, and to the extent they disagree we adjust the model. When using computer graphic methods this approach is called *inverse rendering*, and it is at the frontier of what is currently possible (REFERENCES HERE).
...
This book is written with the belief that achieving this goal will benefit signficiantly from understanding the light field and the instruments that measure it. This belief is not universally held.
:::
