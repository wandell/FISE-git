...
## Light field units
There are many applications that require us to specify the intensity of the light field.  The widely used units are not specified with respect to individual rays or waves; they are all specified with respect to averages over a solid angle (steradian) or a surface ($m^2$). We describe the most common measures of spectral radiance and spectral irradiance here.  The parallel measures for luminance and illuminance can be found in the Chapter @sec-human, that introduces concepts from human vision that are at the foundation of those measures.
