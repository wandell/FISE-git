...
The pinhole diameter is one source of blur. Let's denote the diameter with the symbol $D$. This is what we would expect if light were rays, not a wave. This value is the same no matter how far the image plane is from the pinhole.

The blur we expect for a wave passing through an aperture can be calculated from first principles. It will depend on the wavelength of the light $\lambda$, the diameter of the pinhole, and the distance from the pinhole to the image plane, $L$. The full calculation of the image is described in @goodman2022 (pages 88-94). For the present purpose, we simply show the image in Figure XXX. Notice that the image of the point through the circular pinhole is radially symmetric with a bright spot in the center, surrounded by a set of concentric rings of decreasing intensity. The central region is called the Airy disk, and it contains about 85% of the total energy. The diameter of the disk on the image plane is given by the formula
