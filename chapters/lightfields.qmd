...
There are many applications that require us to quantify the light field. In this section we introduce the fundamental concepts of light field quantification.

The quantification are based on the amount of energy, or equivalently the number of photons, in the light field. In the case of energy the basic unit is joules/sec, which is also called watts. In the case of photons the measure is photons per second. For radiometry, the measurements are always made with separately for each wavelength. For research areas working with light, wavelength is commonly expressed with respect to nanometers [^lightfields-5]. Thus, one expect to find watts per nanometer or photons/sec per nanometer. Additional types of units are incorporated as we define the geometry of the measurement.
...
Light field measurements can be divided into two main types, with a large number of subtypes. The main division concerns whether we are measuring a light field that is radiating from a source (radiance), or we are measuring a light field that is arriving at a surface (irradiance). We will introduce the most common measures of spectral radiance and irradiance here.

Finally, there is a parallel set of light field measurements whose purpose is to summarize the impact of the light field on the human visual system. These measurements, called photometry, take a weighted sum across wavelength of the radiometric units. The weighting with respect to wavelength is determined by the relative impact of each wavelength on the human visual system, as measured in an experimental paradigm called 'flicker photometry'. The photometric terms that parallel radiance and irradiance are luminance and illuminance. In this section, we will describe the radiometric quantities that specify each wavelength. In @sec-human we explain the experimental basis for summing across wavelengths to convert radiometric into photometric quantities.

The units used to specify light field energy are specified as the mean values over a solid angle (steradian, $sr$) or a surface ($m^2$). The physical units are also described independently for each wavelength, which is reasonable because the energy at different wavelengths do not interact. In typical conditions we can measure at each wavelength separately, mix the two lights, and find that the result is simply the sum of the individual measurements.

### Radiance from a point
@fig-radiometry illustrates the critical geometric differences between the various radiometric measures. One important distinction is whether we are considering the light emitted from a location (radiance) or light arriving to a location (irradiance).

There are three principal measures of emitted light. When we consider a point source (upper left), we might measure the energy emitted in all directions, the *radiant flux*. In this case we are summing the energy across all emission directions.  The units are watts measured separately for each small band of wavelengths (watts/nm).  

![The geometry of light field measurements.](../images/lightfields/01-radiometry.png){#fig-radiometry fig-cap-location="bottom" width="100%" fig-align="center"}

We may decide to measure the light emitted from a location in a particular direction.  In that case, we specify the energy of per unit cone of rays in a particular direction.  The steradian measures angles in three-dimensions, just as the radian measures angles in two-dimensions. It measures how wide or narrow a directional spread is from a central point. When we measure the light emitted in a direction, we may be measuring a larger or smaller bundle.  The radiant intensity normalizes the angle we measure to the unit steradian. 

::: {.callout-note collapse="false" title="Steradians"}

![Three-dimensional angle units are called steradians.  One steradian is defined as angle of the cone whose surface area on a sphere of radius $r$ is equal to $r^2$. Because the surface area of a sphere with radius $r$ is $4 \pi r^2$, it follows that the complete sphere surrounding a point is $4 \pi$ steradians.](../images/lightfields/01-radiometry-steradian.png){#fig-steradian width="60%" fig-cap-location="bottom"}
:::

### Radiance from a surface
In many applications, the light we are interested in arises from an extended surface - such as a wall or the side of a large object.  In that case, many points contribute energy to the measurement.  The unit we create is normalized by the surface area of the extended surface and by the angle that surface forms with the measurement instrument.  The unit is called 
