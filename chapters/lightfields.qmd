...
There are many applications that require us to quantify the light field. In this section we introduce the fundamental concepts.

The measurement can be divided into four different general types, with a large number of subtypes. The first division is whether we are measuring the light field at every separate wavelength (radiometry), or whether we are specifying the weighted sum across multiple wavelengths (photometry). In this section, we will describe the radiometric quantities that specify each wavelength.\
The second division concerns whether we are measuring a light field that is radiating from a light source or a surface (radiance), or we are measuring a light field that is arriving at a surface (irradiance). We will introduce the most common measures of spectral radiance and irradiance here.
