...
The first division is whether we are measuring the light field at every separate wavelength (radiometry), or whether we are specifying the weighted sum across multiple wavelengths (photometry). In this section, we will describe the radiometric quantities that specify each wavelength.\
The photometric terms corresponding to radiance and irradiance are luminance and illuminance. In @sec-human we explain the experimental basis for how we sum across wavelengths to convert radiometric into photometric quantities.

The units used to specify light field intensities are not defined with respect to individual rays or waves; they are specified as average values over a solid angle (steradian) or a surface ($m^2$). The physical units are also described independently for each wavelength, which is reasonable because the energy at different wavelengths do not interact. In typical conditions we can measure at each wavelength separately, mix the two lights, and find that the result is simply the sum of the individual measurements. \### Radiance
