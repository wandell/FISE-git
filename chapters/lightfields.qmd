...
Why do we need to sense and perceive our environment? Animals move in search of food and avoid predators by relying on their ability to sense and interpret the environment. Machines, too, sense and interpret the environment in order to identify and grasp objects, as well as to move from place to place. For many animals and machines, the ambient electromagnetic radiation is a key source of information. Immobile life forms and machines that do not move or reach, such as trees and plants, mainly use the radiation as a source of energy; but mobile life forms - a hawk, a rabbit, or a person - encode and interpret the radiation to glean information and interact with the environment.
...
Not all electromagnetic radiation is equally informative. High energy radiation (e.g., X-ray and Gamma ray bands) passes through most objects undisturbed. Low frequency radiation provides limited spatial resolution. Absorption and scattering of electromagnetic radiation in the wavelength range of a few hundred nanometers is useful for informing us about the environment.

The human eye encodes electromagnetic radiation in the wavelength range from about 380 nm to 770 nm; the word light means that portion of the electromagnetic radiation that humans see. Many image systems are designed to measure this visible band; some measure electromagnetic radiation at wavelengths we do not see, thus extending our senses[^lightfields-1].

[^lightfields-1]: See [more about wavebands here.](../supplemental/lightfields-wavebands.html){target="_blank"}.

The electromagnetic radiation fills the environment around us, traveling in many directions and and interacting with most materials. We call the ambient radiation that is in the visible band the *light field*. Leonardo Da Vinci's notebook [@davinciNotebooksLeonardoVinci1970] describes how we know that this field is everywhere. He has a section describing a pinhole camera (camera obscura), made by placing a small hole in a wall of a windowless room that is adjacent to a brightly illuminated piazza (@fig-ayscough). The camera produces an image of the piazza (inverted) on the wall within the room. Leonardo further noted that an image is formed wherever the pinhole is placed. From this observation he concluded that the rays must be present at all of these positions[^lightfields-2].
...
In the following sections we continue to use the light field terminology and follow the signal from the environment, to the entrance pupil of the camera, and then to the sensor.

## Incident light field

An imaging system, say the eye or a pinhole, only records the portion of the environmental light field that arrives at its entrance pupil (@fig-incident-lightfield). It is worth distinguishing that radiation - the *incident light field* - from the environmental light field. For this definition, it is convenient to conceive of the light as comprising rays traveling in space.
...
The rays of the incident light field can be described with a one fewer parameters than the environmental light field because the entrance pupil is just a two-dimensional surface. Thus, the three dimensional position is replaced by the two-dimensional position in the entrance pupil, $(u,v)$.

$$L_I(u,v,\alpha,\beta,\lambda,\rho)$$

If we wish to acquire information about more of the environmental light field we must measure at multiple locations. The human visual system makes two simultaneous measurements of the light field, one with each eye, and this information provides us with stereo vision. We acquire more information about the light field over time by moving our heads and rotating our eyes.
...
The image system optics, whether a pinhole, thin lens, or multi-element lens, transforms the incident light field into a new light field within the camera. This *optical light field* travels from the exit pupil of the optics towards the sensor. It originates in the environment, is reduced to the incident light field, and then transformed by the optics. The camera sensor, or the retina, measures the optical light field.
...
### Calculating the relative amounts of blur
...
The pinhole diameter is one source of blur (@fig-pinhole-basic) and the diffraction based on the wave properties of light is a second source (@fig-ray-wave). The blur we expect based on the ray theory is simply equal to the pinhole diameter, which we denote by $D$. This blur size is the same no matter how far the image plane is from the pinhole.

The spread we expect from the wavefront calculation is more complex and varies with the diameter of the pinhole.  Clearly, if the size of the pinhole is very large, the plane wave will continue along with only some minor interruption at the edge. Thus, the blur due to diffraction will tend to zero.  As the pinhole becomes small, the spread will become much larger approximating the spherical pattern in @fig-ray-wave.  A precise calculation of this spread for a circular aperture was derived by Lord Airy @airy-1835-diffraction.  The formula for different apertures can be found in @goodman-2022 (pages 88-94).

The blur we expect for a wave passing through an aperture can be calculated from first principles. It will depend on the wavelength of the light $\lambda$, the diameter of the pinhole, and the distance from the pinhole to the image plane, $L$. For the present purpose, we simply show the image in Figure XXX. Notice that the image of the point through the circular pinhole is radially symmetric with a bright spot in the center, surrounded by a set of concentric rings of decreasing intensity. The central region is called the Airy disk, and it contains about 85% of the total energy. The diameter of the disk on the image plane is given by the formula
...
Suppose we calculate the size of the diffraction diameter when the image plane is 1 m away, so $L=1$, and for a wavelength of light is 550 nm. The diameter of the Airy disk will be $d = 2.44 \times (550e-9 /D)$. might calculate the diffraction spread on an image plane that is 1 m from the pinhole and the diameter of the pinhole, D. When the ratio exceeds 1, the spread due to diffraction is larger than the spread due to the pinhole. Or make a plot of the size due to diffraction and the size of the hole and show where they intersect.
