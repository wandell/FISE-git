...
In many applications we must measure light from a patch on an extended, flat surface - such as a large light fixture with a diffuser, a wall, or a large display screen. The geometry of thee measurement accounts for two factors. First, we must account for the area of the surface patch, as seen from the detector. For example, if the measured surface patch is a square $0.1$ meters on a side its area is $A_s = (0.1)^2~m^2$. The area, as seen from the detector, can be foreshortened. We calculate this from the angle between the viewing direction and the surface normal, $\theta$. The foreshortened surface area is $A_s \cos(\theta) m^2$. Second, we account for the size of the bundle of rays captured by the detector. We measure this size using the three-dimensional angle, $\omega sr$, of the cone of rays from a surface point that are captured by the detector.
...
The *radiance* $L$, combines the energy ($watts/nm$) with these geometric factors. The standard symbol for spectral radiance is $L(\lambda)$. It has units of watts per nanometer per unit foreshortened area per steradian: \$ \frac{Watts}{nm ~ \cos(\theta) m^2 ~ sr}\$.
