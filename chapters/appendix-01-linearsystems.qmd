...
Image systems engineers often examine system performance by measuring the response to a point of light—this is the point spread function (PSF) introduced in @sec-pointspread.  A point of light, $p$, is represented by a vector that has $1$ in a single location and zeros elsewhere.  Each column of the linear transformation, $\mathbf{L}$, is the point spread function corresponding to a sampled point, $\text{PSF}_p$.

![Matrix tableau of an optical system with the point spreads in the columns.](){#fig-ls-matrix-psf}

I drew this tableau for a one-dimensional system to make the idea clear.  If we are using a two-dimensional model, we would have to flatten the image matrices into columns, but only complexity would be added.  The principles would be unchanged.

## Spatial basis functions {#sec-ls-spatial-basis}
The PSF is a fundamental tool, but it has limitations: using a point of light involves very few photons, and PSFs can vary in complex ways that are hard to summarize with just a few parameters.
