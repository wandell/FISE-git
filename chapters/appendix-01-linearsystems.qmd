...
When a system is not linear everywhere, it may still be well-approximated as linear within a limited region of its input space. In other words, the system is **locally linear**: if you zoom in close enough to a particular operating point, the system’s behavior looks linear, even if it isn’t globally so.

The key idea is that the best linear approximation depends on where you are in the input space. The linear model near $x_1$ might look quite different from the linear model near $x_2$. So, when modeling or analyzing a system, it’s important to know which region you’re working in.
...
A key insight is that images can be described in many ways, not just as a collection of points. Some alternative representations are especially useful for analyzing and characterizing the optical performance of imaging systems. Linear models provide a powerful framework for defining and exploring these alternative image representations. We first introduced linear models in @sec-linear-models, and used them to describe the spectral properties of light fields. Here, we revisit linear models in a more general context, applying them to the spatial intensity of images, $S[u,v]$. A linear model for spatial intensity looks like this:
...
## Image transforms {#sec-ls-image-transforms}
For a linear model to be useful, we need a way to find the weights from the image itself. A function that maps $S(u,v)$ to the weights $w[i]$ is called an **image transform**. If we choose basis functions that are orthogonal to each other, this transform becomes especially simple. Orthogonality means that

$$
0 = \sum_{u,v} B_i[u,v] ~ B_j[u,v]
$$ {#eq-spatial-orthogonal}

for any two different basis functions $B_i$ and $B_j$.

To work with these mathematically, we often “flatten” each basis function $B_i$ (which is a matrix) into a long column vector by stacking its columns. We do the same for the image $S[u,v]$, turning it into a vector $\mathbf{s}$. If we collect all the basis vectors as columns of a matrix $\mathbf{B}$, and the weights as a vector $\mathbf{w}$, we can write the linear model from @eq-linear-model as a matrix equation:
...
$$
\mathbf{w} = \mathbf{B^t s}
$$ {#eq-linear-model-inverse}

In words: just multiply the transpose of the basis matrix by the image vector to get the weights. That’s all there is to it!
...
The second is the **harmonic basis** —that is, sine and cosine waves at different spatial frequencies. Harmonic bases are fundamental for analyzing and understanding many systems, especially those that are shift-invariant. In the next sections, we’ll see why harmonics are so valuable, and how they relate to eigenfunctions and shift-invariant linear systems.
...
Many linear systems—including the ones we focus on in this book—have a complete set of orthogonal eigenvectors. This means we can express any input vector (stimulus) as a weighted sum of these eigenvectors. The real power of this approach is that applying the system to such an input becomes very simple:
...
Harmonic functions (like sines and cosines) are almost eigenfunctions for many imaging systems, which is why they are so useful in practice.
...
Because shift invariance simplifies both analysis and computation, many courses on “linear systems” focus almost entirely on shift-invariant cases.
...
In practice, most imaging systems are linear, but not perfectly shift-invariant everywhere. Instead, they are shift-invariant only within certain regions of the field of view. This is called **local shift invariance**. In optics, such a region is known as an **isoplanatic** patch. Within an isoplanatic region, the PSF is (approximately) the same everywhere, so the shift-invariant analysis applies locally.
