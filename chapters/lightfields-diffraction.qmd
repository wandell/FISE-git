...
The diffraction for a point close to the pinhole is more complex. In that case we must account for the spherical shape of the wavefront from the nearby point. This calculation is named for Fresnel and called [near-field diffraction](https://en.wikipedia.org/wiki/Fresnel_diffraction). In the near-field case, the point spread depends on the point's distance from the aperture.