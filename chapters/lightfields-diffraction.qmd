...
If we imagine light as a collection of rays, it is easy to understand why pinhole optics renders a reasonable image (@fig-ayscough). Some of the properties of the pinhole camera, such as why the image is inverted or how it is blurred, are also well-explained by tracing the rays. Newton's formulation of light as rays is a useful and widely used approximation for many calculations.
...
We have seen that the image is blurred by two effects: the pinhole size and diffraction. We can compare the size of these two effects numerically.
