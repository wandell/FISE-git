...
![A set of one-dimensional image harmonics. They vary in $x$ with $f_y = 0$. The contrast $A$ varies from top to bottom $(1, 0.5, 0.1)$. The $x$-range is $(0,1)$, and the spatial frequencies are $f_x = (1, 2, 4, 8)$. [fise_imageHarmonics1d.m](../code/02Optics/fise_imageHarmonics1d.html){target=_blank}](images/optics/optics-harmonics-1d.png){#fig-optics-harmonics-1d width="60%"}
...
![Video showing a line reconstructed as we add harmonics (cosines) at different frequencies. I only added the contrast terms, keeping the mean the same. See [fise_harmonicsLine](../code/02Optics/fise_harmonicsLine.html){target=_blank}. ](images/optics/lineReconstruct.mp4){#fig-optics-reconstruction-movie width="40%" fig-align="center"}
...
The **optical transfer function (OTF)** describes the complete effect of a space-invariant optical system on an image. It is a complex-valued function that captures both the amplitude scaling and phase shift for every spatial frequency, $\mathbf{f}$. It is defined as the ratio of the Fourier Transform of the input and output

$$
\mathcal{O}(f) = \frac{\mathscr{F}\{g\}(f)}{\mathscr{F}\{i\}(f)} 
$$

The OTF is a complex function of spatial frequency 
$$
\mathcal{O}(f) = s(f) \cdot e^{-i \phi(f)}
$${#eq-otf-definition}

Here, $s(f)$ is the real-valued amplitude scaling factor, and $\phi(f)$ is the phase shift (in radians).
...
$$
\begin{aligned}
g(x)             &= \mathscr{F}^{-1}\big(\mathscr{F}\{i\} \cdot \mathcal{O}(f)\big) \\
\mathscr{F}\{g\} &= \mathscr{F}\{i\} \cdot \mathcal{O}(f)
\end{aligned}
$${#eq-otf-calculation}
...
Many times we are primarily interested in how the system affects the contrast (or modulation) of the input harmonics, without regard to the phase shift. This information is captured by the **modulation transfer function (MTF)**, $\mathcal{M}$. 

$$
\mathcal{M}(\mathbf{f}) = s(\mathbf{f})
$${#eq-MTF-defined}
...
It is always good to check the phase term, as well.  The phase shift (in radians) as a function of frequency is called the **phase transfer function (PTF)**, $\mathcal{P}$ and it is simply

$$
\mathcal{P}(\mathbf{f}) = \phi(\mathbf{f})
...
$$
\mathcal{O}(\mathbf{f}) 
= \mathcal{M}(\mathbf{f}) \, e^{\,i \, \mathcal{P}(\mathbf{f})} .
$$ {#eq-otf-ptf-mtf}

## The OTF and the PSF {#sec-otf-psf}
The point spread function (PSF) and the optical transfer function (OTF) are two fundamental ways to describe a linear, space-invariant system. The PSF describes the system's response to a point input, while the OTF describes its response to harmonic inputs. There is a direct mathematical relationship between them: the OTF is the Fourier transform of the PSF.

Let $h$ represent the PSF and $\mathcal{O}$ represent the OTF. Their relationship is given by:

$$
\mathcal{O}(\mathbf{f}) = \mathcal{F}\{h(\mathbf{x})\}
$${#eq-otf-psf}

where $\mathcal{F}$ denotes the Fourier transform, $\mathbf{x}$ represents spatial position, and $\mathbf{f}$ represents spatial frequency. This simple and powerful relationship means that if you know one, you can calculate the other.

::: {.callout-note collapse=false title="Derivation: From PSF to OTF"}
We can show this relationship by considering a specific input: an impulse.

1.  **Define the Input**: Let the input image, $i(\mathbf{x})$, be a perfect impulse (a Dirac delta function, $\delta(\mathbf{x})$). An impulse is a single, infinitely bright point at the origin.

2.  **Find the Output**: By definition, the output of a linear, space-invariant system to an impulse input is its point spread function $h()$. So, for our impulse input, the output image is $o(\mathbf{x}) = h(\mathbf{x})$.

3.  **Transform to the Frequency Domain**: The Optical Transfer Function is the ratio of the Fourier transform ($\mathcal{F}$) of the output to the Fourier transform of the input.
    $$
    \mathcal{O}(\mathbf{f}) = \frac{\mathcal{F}\{o(\mathbf{x})\}}{\mathcal{F}\{i(\mathbf{x})\}}
    $$

4.  **Substitute and Simplify**: Now we substitute our specific input and output into this definition.
    - The Fourier transform of the input impulse is $\mathcal{F}\{\delta(\mathbf{x})\} = 1$.
    - The Fourier transform of the output is $\mathcal{F}\{h(\mathbf{x})\}$.

    Plugging these into the equation gives:
    $$
    \mathcal{O}(\mathbf{f}) = \frac{\mathcal{F}\{h(\mathbf{x})\}}{1} = \mathcal{F}\{h(\mathbf{x})\}
    $$
    This confirms that the OTF is the Fourier transform of the PSF, as stated in @eq-otf-psf.
...
![Pictures of 2d FFT Amplitude spectrum. [See fise_otf2d](../code/02Optics/fise_otf2d.html){target=_blank}](images/optics/otf2d.png){#fig-optics-otf2d width=70%}

The axes represent the spatial frequency range.  It is common to render the spatial frequency map with the mean ($M$) value in the middle of the image at $(0,0)$. The spatial frequencies range from $[-f_N,f_N]$, where $N$ is the number of samples. The frequency is with respect to the image size.  If the image size is $1$ degree, then $f_1$ is $1$ cycle per degree; if the image is $10$ deg, then $f_1$ is $1/10$ cycle per degree.  Because the original image is real-valued, the complex coefficients must satisfy the regularity that $F(f_x,f_y) = F^* (-f_x,-f_y)$, where $*$ is the complex conjugate.

Slow variations across the image are represented in the center of the plot.  If reduce their amplitude and compute the image, as in the middle column, the image reflects mainly the parts that are rapidly changing.  Conversely, if we retain the center part and scale away the higher frequencies, as in the rightmost column, the image is a blurry version of the original.

![The log10 amplitudes of the two-dimensional Fourier Transform coefficients are shown in the bottom row; the corresponding images are shown in the top row.[fise_otf2dFace](../code/02Optics/fise_otf2dFace.html){target=_blank}](images/optics/otf2dParts.png){#fig-optics-otf2dparts width=70%}
