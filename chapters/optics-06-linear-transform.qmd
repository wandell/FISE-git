...
The **optical transfer function (OTF)**, $\mathscr{O}(f)$, is defined as the Fourier Transform of the output divided by the Fourier Transform of the input. From its definition, we can see it is also the Fourier Transform of the point spread function.

$$
\mathcal{O}(f) = \frac{\mathscr{F}\{g\}}{\mathscr{F}\{i\}} = \mathscr{F}\{h\} .
$$ {#eq-otf-definition}

The OTF is a complex function of spatial frequency, and its real and complex components correspond to the amplitude scaling and phase shift,

$$
\mathcal{O}(f) = s(f) \cdot e^{-i \phi(f)} .
$${#eq-otf-definition-complex}

The term $s(f)$ is the real-valued amplitude scaling factor, and $\phi(f)$ is the phase shift (in radians).
...
$$
g(x) = \mathscr{F}^{-1}\big( \mathcal{O}(f)  \cdot \mathscr{F}\{i\} \big)\\
$${#eq-otf-calculation}
...
![A sweep frequency pattern demonstrates to illustrate the MTF. The input image (top) is a sweep frequency, from low to high frequency, at 550 nm. The input contrast is the same as the local frequency increases. The simulation passes this image through a diffraction limited lens, $f/\# = 4$. The local contrast decreases at the higher frequencies. The red curve, defined as the envlope of the modulating output response, is the system's MTF, $\mathscr{M}(f)$. [See fise_opticsMTF.m](../code/02Optics/fise_opticsMTF.html){target=_blank}](images/optics/harmonics-mtf.png){#fig-optics-harmonics-mtf width=90%}
...
![The log10 amplitudes of the two-dimensional Fourier Transform coefficients are shown in the bottom row; the corresponding images are shown in the top row.[fise_otf2dFace](../code/02Optics/fise_otf2dFace.html){target=_blank}](images/optics/otf2dFace.png){#fig-optics-otf2dparts width=70%}
