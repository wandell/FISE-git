...
2.  **Find the Output**: By definition, the output of a linear, space-invariant system to an impulse input is its point spread function (PSF). So, for our impulse input, the output image is $o(\mathbf{x}) = h(\mathbf{x})$.

3.  **Transform to the Frequency Domain**: The Optical Transfer Function (OTF) is defined as the ratio of the Fourier transform ($\mathcal{F}$) of the output to the Fourier transform of the input.
    $$
