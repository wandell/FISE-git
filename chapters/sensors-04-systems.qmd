...
However, in many other imaging systems, the camera must adjust the lens position to bring objects at different distances into focus. This is especially important in consumer photography, such as for close-up portraits or images of nearby objects. Traditionally, users manually focused cameras by looking through a viewfinder and turning a focus ring until the image appeared sharp. Over time, manufacturers developed mechanisms and algorithms to automate this process, typically assuming that the subject near the center of the image should be in focus.

In classic SLR and DSLR cameras, focusing was achieved mechanically, often using a ring that the user could rotate (@fig-sensor-vcm). These lenses were usually rotationally symmetric, so turning the ring both rotated the lens and moved it closer or farther from the sensor. Most camera lenses are multi-element designs; sometimes all elements move together, but often only a subset shifts to adjust focus. The thread pitch of the focus ring determines how finely the position can be adjusted.

For smartphones and other compact devices, a different approach is required due to space constraints. These cameras also use multi-element lenses, but the assemblies are much smaller. Focus is commonly controlled by a voice coil motor (VCM), which is reliable, inexpensive, and widely used in the industry.

![Cell phone optics. The optics themselves are very compact but still multi-element. The position of the optics is usually controlled by a voice-coil motor.](images/sensors/03-sensor-vcm.png){#fig-sensor-vcm}
...
![Classical autofocus mechanism.](images/sensors/03-focusprinciples-2.png){#fig-focusprinciples-classic width="60%"}
