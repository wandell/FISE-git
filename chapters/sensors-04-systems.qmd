...
where $F$ is the f-number and $T$ is the exposure duration.  Notice that the $EV$ is 0 when the f-number is 1 and the exposure duration is 1 sec. The exposure value decreases as the exposure duration increases or the aperture increases.

In film cameras, when each exposure of the film was expensive, separate devices might measure the the general light level and guide the user to set the aperture and exposure. With electronic systems, however, the scene intensity level could be measured by acquisitions through the sensor prior to acquiring the final image. 
