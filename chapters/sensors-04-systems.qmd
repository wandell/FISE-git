...
## Focus
For many application, a camera system should be capable of adjusting the imaging lens position so that the critical objects are rendered in focus at the sensor.  In the first generation of cameras, this would be done manually by the user, who would look through a view finder and move the lens by hand until the image looked sharp. 

For other applications the lens   For many situations the lens formula (@sec-lens-formula) can be used. That formula reveals that when the object distance is greater than, say, 20 focal lengths the focal distance to the sensor plane is fairly constant. For example, suppose you have an imaging system for driving with a 25 mm focal length, and the camera is intended to identify vehicles or vulnerable road users that are at least 1 meter ($x40$) away, the focal distance will be very close to the focal length.  Recall from @eq-focaldistance, the ideal focal distance can only vary between $\frac{40}{39} 25$ and $25$ mm, a range of only 640 $\mu$m.  The sharpness of the image over that range will not change much - the impact will depend on the size of the aperture (@sec-optics-dof).
