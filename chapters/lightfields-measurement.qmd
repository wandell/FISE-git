...
We have been describing light as rays or waves; these are good models for understanding how light travels through space.  There is another way to characterize the light that is particularly important when we are working with sensors or displays.  When we capture light with an image sensor array (or the retinal photoreceptors), we are measuring the intensity of the wavefront at a particular location in space. The vast majority of signal processing applications are based on images of the light field intensity in a plane. I find it remarkable, as did Feynman and many others, that using just the local intensities in a small part of scene, we can estimate the properties of distant objects, including their shape, distance, reflectance and texture.  <!-- Feynman comment about this? -->
...
This section describes the field, **Radiometry**, that specializes in quantifying the light field. Many applications benefit from quantifying the light field. We may need to know the level of energy to expect when we take a picture. We may wish to know the energy emitted from a display. We may want to know how much light is reflected and absorbed by a surface. This section introduces the basic concepts of radiometric quantification. The standard units used for quantifying the intensity is an important part of image representation in the ISETCam simulation software.
