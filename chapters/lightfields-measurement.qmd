...
To this point we concentrated on how light (electromagnetic radiation) travels.  Modeling the light as rays or waves are both helpful ways to understand light propagation in space and through optics. When we measure an image, either with an image sensor or the retina, the traveling is over.  Instead, we are measuring the wavefront intensity (or number of rays), at a particular location in space and time.  This is another important representation we use to think about the imaging signal. In fact, the vast majority of signal processing applications are based on images of the light field intensity.  

For example, we might measure the intensity of the light across the entrance pupil of the optics and the intensity across the image sensor surface.  We would like to have a model of the transformation from the image at the eye to the image at the sensor.  Or we might want to measure the intensity of the light that is reflected from a surface towards the entrance pupil. This section describes, **Radiometry**, the field that specializes in quantifying the light field intensity at different positions.

<!-- Feynman comment about this? I find it remarkable, as did Feynman and many others, that using just the local intensities in a small part of scene, we can estimate the properties of distant objects, including their shape, distance, reflectance and texture.  -->

 Many applications benefit from quantifying the light field. We may need to know the intensity to expect when we take a picture of a natural scene. Or we may wish to know the light intensity emitted from a display. We may want to know how much light is incident on a surface, and how much is reflected by the surface. This section introduces basic concepts of Radiometry, along with the standard units used for quantifying the intensity.  These units are an important part of image representation in the ISETCam simulation software.
