...
To this point we concentrated on how light (electromagnetic radiation) travels through space and through apertures. For this purpose, it is useful to think of models based on rays and waves. There is another way to think about light which becomes critical when we measure the radiation using an image sensor. These devices measure the wavefront *intensity* (either energy or number of photons) in an image plane, sampling the intensity at a set of locations in space and time. This is an important and very practical representation; the majority of image systems applications are designed to work with image intensity measurements. In this chapter we will develop some tools to characterize the propagation of light intensities.

As an example, suppose we measure the image intensity at two planes: the image plane just prior to the optics and the image plane just prior to the image sensor surface. The relationship between these two images will depend upon the optics, of course. We would like to create a calculation that characterizes the transformation, and thus characterizes the optics. Similarly, we might want to measure image intensity of the light coming from a point in the scene and the image intensity from that light in a plane just prior to the a surface in the scene.

Many of these measurements are quantified using the tools of **Radiometry**, the field that specializes in quantifying the light field intensity at different positions. Other relationships, particularly those involving optics, are described by signal processing calculations that rely on linear systems theory. Many applications benefit from understanding the relationships between image intensities measured at different locations. We may wish to know the light intensity emitted from a display and the corresponding intensity that will arrive at the eye. Or we may want to know how much light is incident on a surface, and how much is reflected by the surface. This section introduces basic concepts of Radiometry, along with the standard units used for quantifying the intensity. These units are an important part of image representation in the ISETCam simulation software. The following section introduces models of the intensity transformations that are used to describe the important case of optics.
