# Light field measurements {#sec-lfmeasurement}
...
The discipline of light field measurement is called Radiometry. The measurements in radiometry are made separately for each wavelength. For people working with light, wavelength is commonly expressed with respect to nanometers [^lightfields-5]. Light field quantification describes the amount of energy, or equivalently the number of photons, present in a portion of the light field. In the case of energy the basic unit is joules per second, (also called watts). In the case of photons the measure is photons per second. Thus, in radiometry one typically finds the units such as (watts per nanometer) or (photons per sec per nanometer). We can describe the energy separately for each wavelength because, in most cases, the energy at different wavelengths are independent: we can measure the energy at two wavelengths separately, mix the two lights, and the result is simply the sum of the individual measurements. This additivity is a critical feature of light behavior in most cases[^lightfields-6].  
