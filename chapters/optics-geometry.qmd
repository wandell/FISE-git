...
## Magnification
The top and bottom lenses are the same apart from their focal length.  Ray tracing the same object at distance $d_o$, you can see that the distance to the image, $d_i$ increases with focal length.  The height of the object is the same $h_o$, but the height of the image increases to $h_{iL}$.

Top focal length = 100 mm.  Bottom focal length = 200 mm.

d_o = 300 mm.  So, d_i = f*d_o/(d_o - f) 

![Increasing the focal length magnifies the image.](../images/optics/lensmaker-magnification.png){#fig-optics-magnification1 fig-align="center" width="80%"}

In @fig-optics-magnification1 the two lenses have the same diameter and thus they capture the same amount of the light field.  The lens with higher magnification spreads the image data over a larger area, and thus the signal is reduced.  We can 
## Zoom
Suppose that the image is captured by a sensor with a fixed size.  Perhaps the sensor has 1000 samples.  The effect of increasing the length but keeping the sensor size the same will be to narrow the field of view.  Thus, we will have 1000 samples over a smaller portion of the object.
