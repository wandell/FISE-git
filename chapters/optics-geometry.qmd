...
We can calculate the focal length of a thin lens with (a) both sides approximately spherical, and (b) embedded in a homogeneous environment. 

Suppose the refractives index of the lens is $n_2$ and of the surrounding medium is $n_1$. Suppose that the radius of curvature of the first surface is $R_1$ and the second surface $R_2$. The focal length of the lens will be

$$\frac{1}{f} = ((n_2/n_1) - 1) ( \frac{1}{R_1} - \frac{1}{R_2}) $$

As you can see from the formula, if we flip the lens so that $R_2$ and $R_1$ exchange, the magnitude of the focal length remains the same (it just changes sign).  So the magnitude of the primary and secondary focal lengths are the same for such thin lenses.
...
## Distance to the image plane (focal distance)
The location of the image plane, where an object is represented in best focus, depends on the distance from the lens to the object. The lensmaker's equation lets us
