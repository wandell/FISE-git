...
## Magnification
The top and bottom lenses have the same diameter but different focal lengths. Ray tracing for an object of the same height, $h_o$ and distance, $d_o$, we see that the image distance, $d_i$ increases with focal length, as does the image height $h_{iL}$.

![Increasing the focal length magnifies the image.](../images/optics/lensmaker-magnification.png){#fig-optics-magnification1 fig-align="center" width="90%"}

We drew both cases with the distance to the object assumed to be $d_o = 0.3m$. The bluish lens has a focal length of $0.1 m$, and the greenish one as a focal length of $0.2 m$. Using the lens formula, the image distance for the top is 

$$d_i = \frac{f*d_o}{d_o - f} = \frac{0.1~0.3}{0.3 - 0.1} = 0.15 m$$

and the bottom as

$$d_{iL} = \frac{0.2~0.3}{0.3 - 0.2} = 0.60 m$$.

The image distance has increased by a factor of $4$. Comparing the similar triangles, we see that the image size also increased by a factor of $4$. Both lenses capture the same rays, but the image in the greenish case is spread out over a larger area. To equate the intensity of the two images, we must capture $4$ times as much of the light field.  We can do this by doubling the diameter of the lens, much increases its area by $4$.

## F-number and radiometry
In @fig-optics-magnification1 the two lenses have the same diameter and thus they capture the same amount of the light field.  The lens with higher magnification spreads the image data over a larger area, and the callout "Magnification calculation" shows that doubling the focal length reduces the intensity by $4$. We can compensate for this by increasing the diameter of the lens by a factor of $2$, which doubles the area of the lens. 

Notice that these two changes, together, preserve the ratio of the focal length and the aperture diameter, which is the f/# of the lens.  The conclusion is that lenses with the same f/# will produce images with the same local intensity.  The larger focal length will be compensated for by the increase in lens diameter.

## Zoom
Some cameras have optics with adjustable focal length. The image sensor, however, is usually a fixed size. Suppose that the sensor has 1000 samples.  We have seen that increasing the focal length enlarges the image.  This means the sensor samples will be measuring a smaller amount of whole image. Thus, we will have 1000 samples over a smaller portion of the object.

Maybe this needs an image from Powerpoint.
