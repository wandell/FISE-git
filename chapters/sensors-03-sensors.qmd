...
Silicon-based CMOS image sensors can detect photons with wavelengths shorter than about 1100 nm. However, the human eye is sensitive only to a narrower range, roughly 380 nm to 700 nm. For consumer photography, the goal is to capture images that appear natural to human observers, so sensors are designed to record light primarily within the visible spectrum. The human visual system samples this spectrum using three types of broadband photoreceptors, making it unnecessary for most cameras to capture more than three broad spectral bands.

To achieve this, two types of spectral filters are used in image sensors: one to block unwanted infrared and ultraviolet light, and another to divide the visible spectrum into three color channels. The resulting data are then processed to produce images that closely match human color perception. In this section, I describe these filters. After reviewing the relevant properties of human vision (@sec-human), I explain how sensor color channels are processed to render accurate or visually pleasing images (@sec-imgprocessing).
...
![The UV/IR blocking filter are placed above the microlens. These filters allow only radiation visible to a person (light) to reach the photodiodes. Source: <a href="https://commons.wikimedia.org/wiki/File:Removed_EOS_350D_IR-blocking_filter.jpg" target="_blank">Source: Removed EOS 350D IR-blocking filter</a>](images/sensors/03-sensor-uvirfilter.png){#fig-sensor-uvir width="80%"} 
...
Below the UV/IR blocking filter, color cameras have a **color filter array (CFA)** (@fig-pixel-layers). Typically, the array is comprised of three types of filters, arranged in a repeating pattern. The smallest repeating unit of the color filter array is called a **super pixel**. The pixels behind one of the color filter types forms a mosaic that samples the image.  Taken together, the pixels comprise four interleaved mosaics. Each of these spatial mosaics is usually called a **color channel**. 

It is common to have three channels, red, green and blue, in consumer photography. The most common super pixel pattern is made from two green filters, one red and one blue (@fig-cfa-pattern). This is called the Bayer pattern, to honor the person who patented this arrangement (@bayerColorImagingArray1976).
...
The data from these interleaved mosaics is ordinarily combined into a full-color image that has a red,green and blue value at every location. This processing is called **demosaicking**.  When images are coarsey sampled, demosaicking is important.  But for modern sensors, which have a very large number of small pixels, and the entire super pixel is within the pointspread function of the lens, demosaicking becomes less important.

@fig-cfa-spectral shows the spectral transmission of three color filters from a consumer camera. Different spectral filters, and even different physical methods, are used in other applications, say, for medical, scientific, or industrial cameras.
