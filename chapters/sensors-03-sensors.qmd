...
For consumer photography, the most common CFA pattern is the Bayer pattern [-@bayerColorImagingArray1976], which uses two green filters, one red, and one blue in each super pixel (@fig-cfa-pattern). The data from these mosaics is combined to create a full-color image with red, green, and blue values at every pixel location. This process is called **demosaicking**. 
...
## The Bayer pattern
<a href="https://en.wikipedia.org/wiki/Bryce_Bayer" target="_blank">Bryce Bayer</a> was a researcher at Kodak Research Labs and the inventor of the Bayer color filter array. 

Kodak was among the first companies to design and market digital cameras. In the mid-1960s, Bayer’s group worked alongside another team focused on psychophysics and human color perception. The Bayer filter was designed to mimic aspects of human vision: specifically, it uses twice as many green filters as red or blue, reflecting the human eye’s greater spatial resolution to these wavelengths.
...
Demosaicking is important when a sensor has relatively large pixels and thus the images are coarsely sampled. For modern sensors with many small pixels—where the entire super pixel fits within the lens point spread function—demosaicking is less critical. We will analyze the impact of demosaicking and pixel size in @sec-imgprocessing.
