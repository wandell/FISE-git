...
In the original planar technology, the photodiode and circuitry occupy adjacent space within the substrate. This means that the photodiode area must compete for space with the transistor size.  Reducing the photodiode area means less light efficiency;  reducing the transistor size means noisier performance.
...
**Stacked sensors** are an advanced design that places the photodiode and CMOS circuitry on different layers, using a method called **die stacking** technology. The layers are bonded together using direct oxide bonding and connected electrically with **Through-Silicon Vias (TSVs)**. Placing the photodiode and circuitry on different layers means they no longer compete for space.  This approach not only improved light sensitivity but also established the foundation for stacking more complex readout and processing circuitry beneath the photodiode layer, paving the way for the stacked pixel architectures that followed.
