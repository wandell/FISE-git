# Sensors {#sec-sensors}
We have been considering the properties of the CMOS imager one pixel at a time. In this section we consider the array of pixels, which means we must consider factors such as variations between pixels, the color filter array, and the microlens array.  I describe basic sensor properties as a foundation.  Then, I discuss recent innovations in the pixel and sensor circuitry. This is an active field, so please take this section as an introduction to the foundations and a guide that leads you to learn about the new developments.

## Sensor noise
When manufacturing an array comprising millions of pixels, there will be minor differences them.  This might include the area of the photodiode, or the capacitance on the floating diffusion node, or perhaps the material doping concentration. Foundries are amazing, so there is not much variation, but there will surely be some slight difference between the millions of pixels. Fluctuations across the array are considered to be **sensor noise**.  Because these variations do not change over time, they are called **fixed pattern noise (FPN)**.  
