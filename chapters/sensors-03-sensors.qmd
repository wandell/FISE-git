...
In traditional (planar) sensor technology, both the photodiodes and their associated transistor circuits are fabricated side by side within the same silicon substrate. This arrangement forces a trade-off: increasing the area for photodiodes improves light sensitivity, but leaves less space for transistors, which can increase noise and limit functionality. Conversely, allocating more area to transistors reduces the light-gathering efficiency of each pixel.
...
Modern CMOS image sensors incorporate several key components to achieve high spatial resolution and accurate color reproduction (@fig-pixel-overview). The microlens array and deep trench isolation technologies both enhance sensitivity and minimize crosstalk between pixels. In addition, sensors include wavelength (spectral) filters that control the spectral sensitivity, enabling the capture and rendering of color images. These filters are essential in consumer cameras and are commonly used in other imaging applications to accurately record the spectral content of a scene.

### Microlens arrays {#sec-sensor-microlens}
The microlens array is a layer of tiny lenses, each equal to the size of the pixel aperture, placed above the color filter array. These lenses redirect the light from the main lens onto the photodiode, increasing the light-gathering efficiency and reducing **pixel cross-talk**. This refers light rays from the imaging lens that arrive at a fairly large angle with respect to the color filter. Such a ray may pass through the filter but end up generating electrons in a pixel that has a different color filter (@fig-components-microlens). Over time, microlens arrays have been used to support additional functions.  They are important for making measurements used in autofocus (@sec-phasedetection-autofocus) algorithms, and they are also important for certain advanced imaging techniques that capture optical light fields (@sec-sensors-lightfield).
