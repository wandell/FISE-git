...
## Terminology: Keeping distances and planes distinct
Students frequently confuse terms that contain the word "focus." For a comparison table and guidance distinguishing **focal length** ($f$), **image distance** ($d_i$), the physical **sensor distance** ($d_s$), and why to avoid saying simply "focal distance," see the resource [Optics Glossary: Basic Distances and Planes](resources/optics-glossary.qmd).
:::
...
## Ideal thin lens {#sec-ideal-thinlens-shape}

If you ever wondered what the shape of a thin lens that is close to ideal (diffraction-limited), have a look at this @sec-resource-optics-ideal-lens.  It is computable and can be built for a single wavelength, but not really for multiple wavelengths.
