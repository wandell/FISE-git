...
where $c$ is the speed of light in a vacuum, $v$ is the speed in the material. Thus, in a vacuum, $n$ is $1.0$, and it is very close to $1.0$ in air.  In some materials, such as glass, the velocity is much slower, so that the index might be $1.3$ or even $1.5$. 

In many common materials the velocity is reduced because the light, which is electromagnetic radiation, induces an opposing electromagnetic signal in the material. This signal is predicted by Maxwell's equations.  There are some materials, such as liquid crystals, whose material interactions produce interesting and useful effects (birefringence) that we will describe in @sec-displays.
