...
In his influential work in the 11th century, "Kitāb al-Manāẓir" (Book of Optics), the Alhazen observed that a spherical lens produced a blurry image on a planar surface. The reason is that the change in ray direction at the sphere edge is too large compared to the change near the center of the lens. Thus the rays do not arrive at the same place at the image plane. We refer to this as a spherical aberration.

It is possible to create an image without spherical aberration in several ways.  In 1854 James Clerk Maxwell,noticing that biological eyes are often spherical, suggested that a way to correct this would be by creating lenses with an index of refraction that varies from the center to the edge. This turned out to be true of many biological eyes. Lenses engineered to have a variable gradient index of refraction are valuable in optical design and called *GRIN* lens. 
...
-->
Another way to reduce spherical aberration, even when the lens is made from one material, is to use a different shape.  There is a class of aspherical lenses that designers frequently use to reduce the spherical aberration. Lens design software searches through the parameters of these shapes, given the materials and other design constraints, to create circularly symmetric aspherical lenses.

\begin{equation*}
z(\rho) = \frac{\rho^2 / R}{1 + \sqrt{1 - (1 + k)(\rho/R)^2}} + A_4 \rho^4 + A_6 \rho^6 + A_8 \rho^8 + \cdots
\end{equation*}

The parameter $\rho$ is the radial distance from the optical axis. The value $z(ρ)$ is called the sag of the surface at a radial distance $\rho$ from the optical axis.   $R$ is the radius of curvature at the vertex of the lens.  $k$ is called the  conic constant, and it determines the overall deviation from the sphere.  The parameter $A_i$ are called higher-order aspheric coefficients that can be adjusted to reduce different types of lens imperfections.
...
Finally, it is possible to change the imaging surface from a plane to a different shape.  If the spherical lens and sensor curvature match appropriately, the spherical aberration will be greatly reduced. This idea is partly inspired by the fact that the retina in many animals lines the interior of the eyeball, forming a curved sensor. Several groups have created sensors on bendable materials, including some very expensive research projects. The idea is to build these sensors for cameras with very large field of view and very high resolution at all angles.
