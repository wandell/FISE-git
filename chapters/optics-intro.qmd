...
When a ray of light crosses from one medium into a second, several things can happen (@fig-reflected-scattered).  The ray might be reflected back, as in a mirror.  Or, the ray might enter into the material, interact with the material in a series of internal reflections, and emerge back in one of many different directions.  Also, the ray might enter the material and be absorbed, giving up its energy to the atoms in the material.


![Several things can happen when rays arrive at the boundary of two mediums. (a) The rays might be reflected. (b) The rays may enter the medium, be internally reflected multiple times, and emerge in one of many different directions (scattering). (Not shown) The ray may might enter the medium and be absorbed.](../images/optics/snell-reflected-scattered.png){#fig-reflected-scattered fig-cap-location="bottom" fig-align="center" width="90%"}
...
The important case for controlling the rays, and thus for optical design, is a different case: the ray enters the new medium and continues on its way. Thios interaction was studied by the Greek astronomer Ptolemy in about the year 150. He documented that a ray that passes from air into, say water or glass, changes direction. We call this effect **refraction**. 

Ibn Sahl in Persia, around 930, quantified the change in direction as light  crossed the boundary between the mediums, and he used this knowledge to design lenses. In 1621 the Dutch astronomer Willebrord Snellius discovered a mathematical formulae for refraction, as did Renee Descartes who published the finding in 1637. Snell's discovery was published later, long after his death. To recognize his priority, the formula for refraction is widely called Snell's Law, though in France it is can called le "loi de Snell-Descartes". Huygens was aware of Snell's law, and he derived derived it based on the assumption that light is a wave [^optics-intro-2].  I think that is cool.
...
The mathematical formulation of Snell's Law is defined using the quantities in @fig-snell-law. A light ray (Incident ray) is shown crossing a planar boundary between one medium (say, air) in a second medium (say, glass). We  define the angle $\phi$ between the incident ray and a line perpendicular to the surface (the surface normal). The refracted ray has a different angle with the surface normal, $\phi '$

![When a ray crosses the boundary between two materials, it may change direction. This occurs because the light wave travels at different speeds in different materials. The material property that describes the speed is the *index of refraction* ($n$ and $n'$). Snell's Law relates the angle of the incident ray (measured from a line perpendicular to the surface) to the angle of the refracted ray.](../images/optics/snell-refraction.png){#fig-snell-law fig-align="center" fig-cap-location="bottom" width="50%"}

For two different media and a wide range of incident ray angles, $phi$, the relationship this simple formula holds:  

$$n~sin(\phi) = n'~sin(\phi')$$

The two terms, $n$ and $n'$ define a property of the medium that is called the index of refraction.  The value of a medium's index of refraction measures the speed of the electromagnetic radiation as it travels in the medium. In a vacuum, $n$ is $1.0$ and it is essentially $1.0$ in air.  In different mediums, such as glass, the index might be $1.3$ or even $1.5$, because the radiation travels much more slowly. The reason for the slow down has to do with the electromagnetic properties of the medium and can be predicted by Maxwell's equations.  There are some materials, such as liquid crystals, whose interactions produce interesting and useful effects that we will describe in the Display chapters.
