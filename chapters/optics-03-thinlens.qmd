...
Notice that reversing the lens, so that $R_2$ and $R_1$ exchange leaves the magnitude of the focal length the same; it just changes sign. This means that the magnitude of the two focal lengths are the same, even if the curvatures differ.
...
-   $N$ is the lens $f/\#$ (focal ratio, dimensionless)
...
**Example:**  
The plot below shows the DOF for a 100 mm focal length, diffraction-limited lens, as a function of $f/\#$ and object distance. The calculations use the ISETCam script [s_opticsDoF](https://github.com/iset/ISETCam/blob/main/scripts/optics/focus_dof/s_opticsDoF.m){target="_blank"}, which calls [opticsDoF](https://github.com/iset/ISETCam/blob/main/opticalimage/optics/opticsDoF.m){target="_blank"} to implement @eq-optics-depthoffield.

![Depth of field in meters (z-axis) at different object distances and thin lens approximation $f/\#$ values. ](images/optics/optics-dof-image.png){#fig-optics-dof width="60%"}
