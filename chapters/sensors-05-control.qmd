...
::: {.callout-note collapse="true" title="Optimal SNR from Exponentially spaced reads"}
...
As the table shows, the regression estimator has **lower SNR** than the last sample for small \( k \). When samples are exponentially spaced in time, most of the signal is concentrated in the last read. Early reads contain proportionally less signal but still contribute read noise, diminishing the benefit of regression.
