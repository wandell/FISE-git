...
# Part III: Sensors {#sec-part-sensors .unlisted .unnumbered}
{{< include "includes/WIP-callout.qmd" >}}

This part of the book explains how the analog light image formed by the optics is converted into a digital signal. This conversion involves two fundamental challenges:

1. **Photogeneration (Physics):** Converting incident photons into mobile electrons inside silicon via the photoelectric effect. The quantum nature of light dictates the fundamental limits of sensor performance (shot noise), a physical process that is identical whether in a CCD or a CMOS sensor.
2. **Readout Architecture (Engineering):** Collecting, transporting, and measuring those tiny charge packets across an array of millions of pixels. We trace this engineering journey from Willard Boyle and George Smith's charge-domain shifting in charge-coupled devices (CCD) to modern voltage-domain CMOS active-pixel architectures.

We then define the key parameters that characterize sensor performance, such as quantum efficiency, read noise, and dynamic range. The final chapters describe the complete sensor system, including color filter arrays and control electronics, and survey the landscape of modern sensor innovations.
