...
## Measuring light fields - overview {#sec-lfmeasurement-overview}

When we quantify the amount of electromagnetic radiation, we are making a **radiometric** measurement. The amount can be specified in units of energy or the number of photons. If we measure how the amount varies over an array of sampled locations, we are measuring an image. The image might represent the radiation incident on a surface (such as a photograph) or encoded by a sensor array. The title of this book, foundations of **image** systems engineering, acknowledges that measuring light intensity as an image is central to the field.

Many aspects of image systems engineering are framed in terms of images, rather than radiation as rays or waves. For example, suppose we measure the image at two planes: the plane just prior to the optics and the plane just prior to the image sensor surface. The relationship between these two images depends upon the optics. We would like to model how the optics transforms one image into the other. This transformation characterizes the optics. 

As a second example, we may wish to know the correspondence between the image emitted from a display and the corresponding image that arrives at the retina. Or we may want to know how the intensity of radiance that is incident on a surface, and the intensity of the radiance that is reflected by the surface. These calculations do not require us to think of rays or waves. We can develop tools and models based entirely on measurements of intensity made by radiance sensors.

The key principles for defining systematic measurement of image intensities are from **Radiometry**, the field that specializes in quantifying the light intensity at different positions. This section introduces basic concepts of Radiometry, along with the standard units used for quantifying the intensity. Radiometric units are an important part of image representation in the ISETCam simulation software. In @sec-optics-linear-space we introduce models of the intensity transformations, particularly for the important case of characterizing optics. Those radiance calculations rely on the properties of images and rely heavily on linear systems theory.
...
![**The geometry of four radiometric measures.**](images/lightfields/02-measuring/01-radiometry.png){#fig-radiometry fig-cap-location="bottom" width="100%" fig-align="center"}
...
*Maybe this goes first and is 'measuring light fields'.  Then we add radiometric measurements next.*

Radiometry informs us about the amount of light, often sampled over space. It is equally valuable to measure the intensity of the rays of light that make up the light field, traveling in space.  This might be the rays of the environmental light field or the the rays of the optical light field.
