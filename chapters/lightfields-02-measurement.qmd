...
Perhaps most important for image systems engineering is to think about the data collected by an image sensor. For many years, the most common types of sensors were comprised of many small regions (**pixels**) that spatially sampled the irradiance at the sensor surface. Each pixel would sum the energy from the rays in all direction, and it would also sum across rays of a selected band of wavelengths. The number of spatial samples has increased steadily over the decades, starting with about $256 \times 256$ samples and now containing tens of millions of samples. Even from the earliest days, engineers and scientists realized that it might be beneficial to measure the rays from different directions separately (@fig-adelson-wang), and it was immediately recognized that to reproduce color photographs required pixels with different wavelength sensitivities (@maxwell1872-colorvision).

To some extent, image systems all comprise some method of measuring the light field combined with algorithms to interpret the light field. There are many ways to both acquire and interpret, and the remainder of the book is filled with examples.
