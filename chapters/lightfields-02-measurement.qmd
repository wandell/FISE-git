...
First, we explain measurements of the direction of the light rays at a detector. The redirection of the rays is an essential function of an optical instrument, and the unwanted redirection of rays as they pass through a turbulent medium (such as the atmosphere, or water) is an important part of image system characterization. As we develop the ideas of optical instruments and their design, we often understand them in terms of how they redirect the rays in the light field.

To measure light we also need to go beyond the concept of a ray. A single ray is only an idealization, it has no energy. But a great deal that matters to us about light is how much energy it has.  To make such measurements we must integrate the light over a small area, or over a small solid angle.  We also integrate over a finite wavelength band and a finite time interval. 

**Radiometry** is the field that provides the quantitative framework for how we make these integrative measurements that specify the energy. Radiometry works primarily with spectral (per-wavelength) quantities and SI[^SI-units] units (e.g., watts or photons per second), which are standard in imaging and used by simulation tools such as ISETCam. The next sections define the core quantities — radiant flux, radiant intensity, radiance, and irradiance— and show how the geometry of the imaging framework (area, angle, and foreshortening) enters their units.
...
How do we measure the direction of the rays in a light field? A pinhole camera offers a simple answer. Each point in the image from a pinhole camera measures the intensity of an incident light field ray at different angle. The measurement is at the location of the pinhole; to learn more about the light field we follow Leonardo's lead and move the pinhole to different locations. This accumulates information about the extended light field.

A slightly different way to think of it is to consider a small lens (called a *lenslet*). We might measuring just a small region of the light field in which the rays are traveling approximately in the same direction.  We say the rays in such a small region are *collimated* (parallel to one another).  In that case, the lenslet will focus the rays focuses onto a sensor at a small point whose position of the point depends on the angle of the incident rays.

![A lenslet can be used to measure the direction of rays in a small region of the light field. The green lines show rays parallel to the main axis of the lenslet: the image point is centered.  The red rays arrive at an angle, and their image will be displaced from the center by an amount that depends on the angle.](images/lightfields/02-measuring/lenslet-0.png){#fig-lenslet width="60%"}

### Shack-Hartmann wavefront sensors
The fundamental way to characteriz how an optical system transforms an incident light field into an optical light field (@eq-lightfield-optical) is illustrated in @fig-shack-hartmann. The instrument used to measure the wavefront is called a Shack-Hartmann wavefront sensor.

![Principles of the Shack-Hartmann wavefront sensor.](images/lightfields/02-measuring/02-Shack-Hartmann.png){#fig-shack-hartmann width="100%" fig-align="center" .margin-caption}

Assembling an array of lenslets, each with a sensor beneath it, samples the light field across space. The location of the image on the sensor informs us about the directions of the rays in that part of the light field.  Assuming the rays in each small region are collimated, and we sample finely enough, we get a reasonable estimate of the direction of the rays.

### Wavefront sensor applications
The wavefront sensors are used in applications far beyond optical measurement. A critical enabling technology for several applications is that we can change the measured ray directions using a small deformable mirror, in which the slant the mirror by an amount to change the direction of the rays.

For example, astronomers measure the wavefront of light from a guide star at the top edge of the atmosphere. The rays from this small point of light would ordinarily be parallel when they arrive at the telescope.  But the atmosphere distorts the wavefront. They measure the actual wavefront and couple it with a wavefront corrector to compensate for the atmospheric distrotions.  These are called adaptive optics systems (@sec-adaptive-optics), and they enable creating a much sharper image of the sky, limiting the distortions from the atmosphere.
...
### Light field cameras
The lenslet-array principle used in wavefront sensors can also be built into an ordinary image sensor. Placing a lenslet array in front of the sensor, so that each lenslet covers a small group of pixels, lets the sensor record both the position of a ray (which lenslet) and its angle (which pixel beneath it)—directly sampling the optical light field (@eq-lightfield-optical). @adelson-wang-1992 introduced this design and showed how the recovered position-angle information supports depth estimation; Ren Ng and colleagues later commercialized it as the Lytro camera, enabling users to refocus an image after it has been captured [@ng2005-lightfield]. We describe the sensor design in detail, and its relation to modern autofocus methods, in @sec-sensor-lightfield.
...
Capturing directional information—measuring rays from different directions separately, as in a light field camera (@sec-sensor-lightfield)—yields richer data and is the basis of light field imaging.  The geometry of pixels, including small lenslet arrays placed over the pixel arrays, enables us to make useful measurements about the light field.
