...
When measuring light fields, the quantity of interest is the direction and intensity of the rays (c.f. @eq-lightfield-environment, @eq-lightfield-incident, @eq-lightfield-optical). The intensity of the rays is specified either in units of energy (watts = joules per second) or the number of photons (@sec-photon-discovered). The first part of this chapter begins by describing instrumentation to measure the ray directions and intensities in a light field.  This instrumentation has applications in a wide range of fields, including ophthalmology and astronomy.

In many other cases, we wish to measure the total amount of light at different positions. If we measure how the amount varies over an array of sampled locations, we are measuring an image. The image might represent the radiation incident on a surface (such as a photograph) or encoded by a sensor array. The title of this book, foundations of **image** systems engineering, acknowledges that measuring light intensity as an image is central to the field.

Many aspects of image systems engineering are framed in terms of images, rather than rays or waves. For example, suppose we measure the image at two planes: the plane just prior to the optics and the plane just prior to the image sensor surface. The relationship between these two images depends upon the optics. We would like to model how the optics transforms one image into the other. This transformation characterizes the optics. 
