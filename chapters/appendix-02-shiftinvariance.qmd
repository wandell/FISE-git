...
The matrix multiplication for a shift-invariant system, which we visualized as a sum of scaled and shifted point spread functions, is a fundamental operation known as **convolution**.
...
Here, the output at position $n$ is a weighted sum of the input values, where the weights are determined by a "flipped and shifted" version of the PSF. This formula is the mathematical expression of the process shown in the SIL matrix tableau.

Convolution is the essential calculation for determining the output of any SIL system in the spatial domain. The next sections explain why we often prefer to analyze these systems in the frequency domain, where this computationally intensive summation is replaced by simple multiplication.
...
The fact that harmonics are eigenvectors of shift-invariant linear (SIL) systems leads to one of the most powerful results in signal processing: the **Convolution Theorem**.

In the spatial domain, calculating the output of an SIL system requires performing a convolution—summing the input signal with shifted and scaled versions of the point spread function (PSF). This operation can be computationally intensive.
...
O[n] = (I * h)[n]
$$
Then, taking the Discrete Fourier Transform (DFT) of each term gives:
$$
\text{DFT}\{O[n]\} = \text{DFT}\{I[n]\} \cdot \text{DFT}\{h[n]\}
$$
...
2.  **Calculate** the output of an SIL system efficiently: transform the input to the frequency domain, multiply by the OTF, and transform back.
3.  **Design** systems by specifying a desired OTF.

Now that we understand the power of this theorem, we can delve into the details of the tool that makes it possible: the Discrete Fourier Series.
