...
The complex representation is useful because we can use it to calculate how an image, $I(x)$, is transformed to its output $O(x)$.  We do this by writing the image using its Fourier representation $\mathscr{F}(I(x))$. We calculate the final image by multipling the Fourier representation by the OTF, and then inverting the Fourier transform back into spatial coordinates.

$$
O(x) = \mathscr{F}^{-1}(\mathscr{F}(I(x))~OTF(f))
$$ {#eq-otf-calculation}

Thus, the OTF contains all of the information needed to describe the system just as the point spread funciton does.
