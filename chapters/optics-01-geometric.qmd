...
@fig-pinhole-image shows the case when the pinhole is fairly small.  In that case, each image point will only receive a ray from a small part of object space.  If we increase the pinhole size, then each image point will receive rays from multiple objects.

![Why enlarging the pinhole increases the blur.](images/optics/06-geometric/pinhole-blur.png){#fig-pinhole-blur width=60%}

When multiple points on the object side contribute to each point on the image side, the result is a blurry version of the world. The general idea, then, is that reducing the pinhole size should -according to the ray model- sharpen the image.

### Parallel rays
The rays from a point generally radiate in a wide range of directions. In many practical cases, the points we are imaging are relatively far away. Consider the distant point, **A** in @fig-pinhole-distant. Only a small angular bundle of the rays from A arrive at the pinhole aperture. These incident rays will be very nearly parallel to one another. According to the ray model of light, this collection of rays will pass straight through the pinhole aperture and form an image of the point on the recording surface that is about the same size as the pinhole itself.
