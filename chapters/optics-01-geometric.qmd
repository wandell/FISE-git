...
Optical modeling of light as rays is a useful and widely used approximation for many calculations.  This method of approximating optical systems relies on many simple geometric calculations, such as the extensive use of similar triangles, that relate the passage of light rays and basic properties of simple lenses. For this reason this modeling approach is also called **geometric optics**.  We will develop the tools of geometric optics in the next few chapters, before we turn to linear systems theory and wavefronts (Fourier optics).

In this chapter we consider the simplest optics, the **pinhole**. After understanding the logic of modeling using rays and pinholes, I explain why the limitations of the ray model and why we will also be modeling optics using light as waves, rather than rays. Finally, we discuss the key concept of the **point spread function**, which plays a role in all the different modeling approaches.

## Pinhole optics
Suppose we imagine light as a collection of rays: then, it is easy to understand Leonardo's explanation of why imaging through a pinhole produces an image. Consider a point on the image plane, and draw the ray that can exit through the pinhole into object space (the colored circles). Each point on the image can receive a ray from a different location on the object side.  The image that is formed will be upside down (and right-left inverted). 

### Pinhole image formation
The basic idea of the pinhole camera (also called the **camera obscura**) has been known for thousands of years. The explanation of image formation is illustrated in @fig-pinhole-image, which shows a set of *objects* on the left and a dark chamber on with an *image* on the right.  The idea is that the rays travel in a straight line, with only a small subset of the rays from each object passing through the pinhole. The geometry illustrates how the rays from adjacent objects arrive at adjacent positions in the image plane, forming a reversed and inverted image.

![A pinhole makes an image.](images/optics/06-geometric/pinhole-image.png){#fig-pinhole-image width=60%}

When the pinhole is small each image point receives rays from a small region in object space. If we increase the pinhole size, then each image point will receive rays from multiple objects. In this case, multiple objects contribute rays to each image point, resulting in a blurry version of the objects. Reducing the pinhole size should -according to the ray model- sharpen the image.
...
The rays from a point generally radiate in a wide range of directions. A special case, but one that is important in many practical applications, is when the rays are from points that are relatively far away (@fig-pinhole-distant). In that case, a small angular bundle of the emitted rays arrive at the pinhole. These rays are nearly parallel, and we say the beam of light is **collimated**.  For an object point, **A** that is aligned with the pinhole, the ray model of light predicts that the rays will continue straight through the pinhole aperture and form an image that is very close to the same size as the pinhole itself.

![Pinhole camera geometry. (A) The rays from a distant, on-axis point arrive at in parallel at the pinhole. Were there no diffraction, the rays would continue on to form an image the size of the pinhole. (B) Rays from a distant off-axis point also arrive in parallel, but they are at an angle to the pinhole aperture. Hence, fewer rays pass through the aperture, making the image dimmer and the blur smaller. For some pinhole sizes, this design is workable. But at a certain size, diffraction makes this design unworkable.](images/optics/06-geometric/pinholePSF.png){#fig-pinhole-distant fig-align="center" width="50%"}
...
@fig-ayscough by Ayscough illustrates an image formed by a complex scene, with many points. Each point in the scene is blurred and rendered at a unique position. Also, each point will be impacted by its relative angle to the pinhole and image surface. If the image intensity through the pinhole is adequate, and we do not mind the blurring, we will have a satisfactory image. It is possible to improve the image sharpness by forming the image on a curved surface , reduing the off-axis blur is reduced.

### Pinhole: Computer graphics
Pinhole cameras are often used in computer graphics calculations. They offer a very simple computational approach to tracing light from the scene onto the recording surface (film or sensor). In computer graphics it is fairly common to aim to render a nice looking image, rather than a physically accurate image. The pinhole approximation to optics provides a very sharp image with large depth of field that is suitable for many applications.

It is also possible ot use pinhole computations to illustrate the limitations of this model in real image systems. In @fig-pinhole-chessset I rendered a chess set scene with the pieces, each a few cm tall, positioned about 0.5 meters from the pinhole camera. The four pictures were rendered using different pinhole diameters. The upper left allows only one ray through from each location in the scene. The aperture diameters for the next three pictures are for three different pinhole sizes. The images are brighter as the pinhole size increases. For this example, the image at the two smaller pinhole sizes are tolerable. Depending on your viewing distance from the screen or page, the third pinhole image might be usable. But the largest pinhole, which lets in the most light, loses so much spatial information that the individual pieces blur together in the image (@sec-airy-pattern).
...
Freeman and Torralba examples in here.

Maybe the video from X, as well.

::: {.callout-note title="Pinholes over time" collapse=false}
Over the centuries, many people surely noticed that when it is possible to form an image by using a pinhole. The Chinese philosopher Mozi (c. 470–391 BC) wrote that an inverted image is formed through a pinhole. He explained the phenomenon by positing that light travels in straight lines, which is why a small opening can create a reversed, upside-down image of an object. The Chinese scientist Shen Kuo (1031–1095) described the process explicitly in his book Dream Pool Essays, noting that the image is inverted because light rays travel in a straight line from the source to the pinhole and then continue straight to form the image.  The most significant figure is Ibn al-Haytham (c. 965–1040), also known as Alhazen. In his influential work, the Book of Optics, he provided a comprehensive analysis of the camera obscura phenomenon. He was the first to use the term al-bayt al-muthlim ("the dark room") and conducted experiments to demonstrate that a small hole could project an image of an external scene onto the opposite wall.  Al-Haytham correctly reasoned that the pinhole created a clear image because it isolates and channels individual light rays from different points of the object, preventing them from mixing. His work laid the groundwork for modern optics and was translated and studied widely in Europe.
:::
...
Mammals eyes aren't usually pinholes - they have a pupil and optics (cornea, lens). But when the pupil is tightly closed down the optics matters little, and the system's optics approach those of a pinhole.  
