...
## Pinhole optics
If we imagine light as a collection of rays, it is easy to understand why imaging through a pinhole produces an image. Consider a point on the image plane, and draw the ray that can exit through the pinhole into object space (the colored circles). Each point on the image can receive a ray from a different location on the object side.  The image that is formed will be upside down (and right-left inverted). 
...
If the pinhole is small, each image point receives rays from a small region in object space. If we increase the pinhole diameter, each image point will receive rays from multiple objects. If multiple objects contribute rays to each image point, the image will be blurry. According to the ray model, reducing the pinhole size should sharpen the image.
