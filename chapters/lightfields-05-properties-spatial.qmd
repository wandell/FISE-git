...
![Consider a single contrast pattern shown as the curve at the top of both panels. Panel (A) shows a camera that images a pattern that is close to a harmonic at frequency $f$ from a distance $d$; panel (B) shows the same camera imaging from a distance $d/2$. Panel (B) images only a portion of the pattern. When we render the image in the first case the spatial frequency of the pattern is still about $f$, but in panel (B) the truncated and rendered image has a lower spatial frequency of about $f/2$.](images/lightfields/05-spatial/one-over-f-1.png){#fig-one-over-f-1 width="60%"}
...
The coherence loss and spectral broadening produce the empirical $1/f^\alpha$ amplitude falloff. You can view the full calculation, waveforms, and experiment with different parameters, in the ISETCam file linked here [fise_spatialCoherence1d.html](https://htmlpreview.github.io/?https://github.com/ISET/isetfise/blob/main/fise/01Lightfields/spatial_coherence/fise_spatialCoherence1d.html){target=_blank}.
:::
...
The machine learning community arrived at diffusion through non-equilibrium thermodynamics (@Sohl2015-diffusion) and score-based modeling (@Ermon2019-diffusion, @Ho2020-denoising). The computational imaging community arrived at the exact same destination from the opposite direction: through empirical Bayes and natural image statistics (Kadkhodaie & Simoncelli, 2020/2021).
