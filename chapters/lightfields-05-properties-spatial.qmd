...
As widespread as this idea is - and I wouldn't start with the idea were it not widely cited - the principle has significant limitations that bother me. First, let's consider whether the principle applies to the light field or to acquired images. If we are describing images, then clearly the optics and sensor noise will have an impact on the relationship [@zoran2009-scale-invariance]. That's a small nit.

More fundamentally, representing an image using harmonics, functions that span the entire field of view, is not great. Consider a spatial pattern for an image measured at a distance $d$; suppose the pattern roughly matches a spatial frequency of $f$ cycles per degree (@fig-one-over-f-1). 

![First one-over-f image description](images/lightfields/05-spatial/one-over-f-1.png){#fig-one-over-f-1 width="50%"}

Next, we measure the same image at a distance $d/2$ (yellow shaded distance). Because we are half the distance away, the same visual angle corresponds to half the distance on the image.  The angle is the same, but we only see half the cycles, so the pattern is $f/2$ cycles per degree. Remember, this is the same pattern.  Thus, we expect that the contrast at $f/2$ in the closer image simply equals the contrast of $f$ at distance $d$. But the one-over-f rule is that the contrast of the lower frequency ($f/2$) should be higher. 

We have a lot of data in the literature suggesting that the contrast measurement rule is true. Yet, we have the logic of the measurement to suggest it makes no sense.  Why would the contrast at nominally corresponding physical spatial frequencies change?

The answer lies in the fact that the field of view has changed, and with it the **spatial coherence length** of the contrast across the measurement window. When we are close to an object (at $d/2$), the object fills a large fraction—or even the entirety—of our field of view. The pattern is spatially coherent across that whole window, so the global Fourier transform registers high contrast. But when we step back to distance $d$, the field of view expands to encompass a broader scene filled with other objects, textures, and background. The original pattern is now confined to a small patch of the visual field; across the rest of the expanded image, its contrast is not coherent. Because the Fourier transform integrates over the entire field of view, this lack of spatial coherence across the wider aperture dilutes the measured amplitude at that higher frequency.

The traditional framing of the $1/f$ law misses this point entirely. The falloff does not arise because natural scenes intrinsically consist of infinite harmonic waves whose power falls off as $1/f^2$. Rather, it arises because image contrast does not remain coherent across large visual angles. As we zoom or change distance, the coherence length at each spatial frequency scales with object size relative to the field of view. 
