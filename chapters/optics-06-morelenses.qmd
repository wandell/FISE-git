...
## Lenses and ray transfer overview  {#sec-optics-morelenses-overview}
...
The BFL tells you exactly where to place a sensor to capture a sharp image of an object at infinity. (See the [Optics Glossary](resources/optics-glossary.qmd) for a summary of how focal length, conjugate image distance, and physical sensor distance relate.) Notice that when $d = f_1 + f_2$, the denominator is zero, and the BFL becomes infinite, which is consistent with the parallel rays produced by a focal telescope.
...
:::{#fig-classic-lenses .figure}
:::{.panel-tabset}
# Tessar 
![[Cooke became Tessar lens](https://en.wikipedia.org/wiki/Tessar). An evolution of the Cooke Triplet, adding a fourth element to improve performance and correct aberrations. [Source: wikipedia](https://en.wikipedia.org/wiki/Tessar#/media/File:Tessar.png)](images/optics/09-morelens/tessar-wikipedia.png){#fig-optics-tessar width="70%"}

# Petzval 
![[Petzval lens](https://en.wikipedia.org/wiki/Petzval_lens) for portraits, known for its large aperture but limited field of view. It's still used in some specialized applications. Big fight.  Biotar is another portrait lens.[Source: Wikipedia](https://en.wikipedia.org/wiki/Petzval_lens#/media/File:Petzval.png)](images/optics/09-morelens/petzval-wikipedia.png){#fig-optics-petzval width="70%"}

# Fisheye 
![The [fish eye lens. CC BY-SA 4.0, ](https://commons.wikimedia.org/w/index.php?curid-74400757)](images/optics/09-morelens/fisheye-wikipedia.png){#fig-optics-fisheye width="70%"}
:::
A collection of classic lens designs.
:::
...
![Ray transfer matrix parameter definitions. (A) A ray is defined by its height $y$ and angle $\theta$. By convention, counter-clockwise angles are positive. (B) A surface is defined by its radius of curvature $R$.](images/optics/09-morelens/definitions.png){#fig-raytransfer-definitions width="50%"}
...
![The input rays are parallel to the optic axis, $\theta_1 = 0$. The output ray angles change proportionally to the input ray field height, $y_1$. According to the sign convention (@fig-raytransfer-definitions), the constant of proportionality is negative.](images/optics/09-morelens/yPhi-equation.png){#fig-yPhi width=50%}
...
![Back focal length and Effective focal length of a black box system.](images/optics/09-morelens/efl-bfl.png){#fig-efl-bfl width=75%}
...
A number of groups have worked to put together functions that extend the ABCD formulation to a much broader mapping, which @goossens2022-raytransferfunctions labeled the **Ray Transfer Function**.  Some groups mapped the rays using polynomials (Hopkins, @hullin2012-xn), and others have worked with neural networks (@Xian2023-cv). 

The future of lens manufacturing and design is likely to include metalenses (@sec-optics-metalens) based on nanoscale structures that cannot be modeled by ray training, it will become increasingly important to develop tools to specify the ray transfer function of optical components.  The development of this infrastructure is a large, and interesting, task for the future.
