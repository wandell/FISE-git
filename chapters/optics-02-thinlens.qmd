...
::: {.callout-note title="CoC formula" collapse="true"}
...
Consequently, the diameter of the circle of confusion is $A \frac{f_X - f_P}{f_X}$
:::

### Formula for depth of field

$$
\text{DOF} \approx \frac{2 N C (d_o)^2}{f^2}
$$ {#eq-optics-depthoffield}

-   $N$ - Lens $f/\#$ (dimensionless)
-   $C$ - Diameter of the circle of confusion (m)
-   $d_o$ - Distance to the subject in object space (m)
-   $f$ - Lens focal length (m)

![Depth of field in meters (z-axis) at different object distances and then lens approximation $f/\#$ values. The calculation is done assuming a 100 mm focal length diffraction-limited lens. See [s_opticsDoF](https://github.com/iset/ISETCam/blob/main/scripts/optics/focus_dof/s_opticsDoF.m){target="_blank"}](images/optics/optics-dof-image.png){#fig-optics-dof width="60%"}
