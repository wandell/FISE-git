...
![A schematic concept of a light field camera. An array of macropixels is suggested at the bottom. Each macropixel comprises a pinhole or a small lens (lenslet) along with a sensing surface below divided into multiple subpixels. Rays from the imaging lens (which in reality would be quite far away) arrive at the pinhole from different angles. The rays pass through the pinhole and are recorded by different subpixels below. The ray's position is determined by the pinhole and its angle is determined by the subpixel that records it. The  position and angle information, collected across the multiple macropixels, can be used for many useful image functions including refocusing or depth estimation.](images/lightfields/01-lightfields/lightfieldPixel.png){#fig-lightfield-pixel .margin-caption width="80%"}
