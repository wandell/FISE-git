...
How do we measure a light field (@eq-lightfield-environment, @eq-lightfield-incident, @eq-lightfield-optical)? If we accept the ray model of light as an approximation, the image generated by a pinhole camera is a measurement of the incident light field at a single location.  The intensity at each location in the image measures the intensity of a small bundle of rays coming from a single direction. If we have an array of pinholes, or if we move the pinhole and sample many locations, then we accumulate more information about the environmental lightfield. The intensity of the rays at each image location can be specified either in units of energy (watts = joules per second) or the number of photons (@sec-photon-discovered). 

Each of the the cameras in @fig-camera-array, measures an incident lightfields and the array as a whole samples the environmental light field coarsely.  But using these cameras adds a further complexity of interposing a lens.  Thus, the light at each image point is summed from rays across the lens aperture. This integration was not a problem for the purpose of computer graphics rendering.  In that application the true goal is to estimate the image at different locations and then interpolate between the positions.

The camera arrays are a very coarse approximation of a light field measurement. There are cases in which we are interested in making a careful measurement.  A particularly important case is characterizing the optical lightfield produced by a lens (@eq-lightfield-optical). For example, astronomers often measure light from a remote object -a star or galaxy- through a lens.  They would like to know what happens to the parallel rays from that object when they pass through the lens. They use this knowledge to design instrumentation so that the rays converge to a small point. Measuring the optical lightfield is also less daunting than measuring the environmental light field. The region that we need to span is very confined compared to the whole environment!

### Shack–Hartmann wavefront sensor {#sec-wavefront-sensor}
Joseph Hartmann suggested a simple and clear approach to measuring the optical light field. Placing an array of pinholes behind the lens, and measure the image behind each pinhole. As I observed above, the image through a pinhole is really the same as measuring the intensity of the rays at the  pinhole from each angle. 
