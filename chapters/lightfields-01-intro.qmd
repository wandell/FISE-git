...
-->
![A light field sensor captures both the position and angle of incoming light rays. The sensor is composed of an array of "macropixels" (bottom). Each macropixel consists of a microlens (or a pinhole, as simplified here) and a group of pixels beneath it. The microlens's location on the sensor determines the ray's position. The specific pixel that detects the light under that microlens determines the ray's angle of arrival. By capturing this position and angle data across the entire sensor, a light field camera can perform powerful computations, such as refocusing an image after it has been taken. (My sketch; Gemini 2.5 Pro helped.)](images/lightfields/01-lightfields/Gemini_lightfieldPixel-Circle.png){#fig-lightfield-pixel width="80%"}
