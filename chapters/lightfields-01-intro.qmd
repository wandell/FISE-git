...
James Clerk Maxwell unified these discoveries in a set of mathematical equations now known as [**Maxwell’s equations**](https://piggsboson.medium.com/james-maxwell-and-how-his-equations-shaped-the-modern-world-8e3e9f6b46c1){target=_blank}. He demonstrated that oscillating electric and magnetic fields behave as waves that propagate through space. Remarkably, the predicted speed of these waves matched the measured speed of light. Maxwell concluded that light itself is an electromagnetic wave, thereby uniting electricity, magnetism, and optics into a single theoretical framework: electromagnetic radiation spanning a range of wavelengths.
...
A sensor design that directly captures the optical light field was invented by @adelson-wang-1992. They placed a lenslet array in front of a standard camera sensor. Each lenslet covers a small group of pixels. As @fig-adelson-wang illustrates—using a pinhole array for simplicity—rays arriving at a lenslet from different angles are focused onto different pixels behind it. This allows the camera to record both the position of a ray (which lenslet it hit) and its angle (which pixel it hit). @adelson-wang-1992 describe how to use this information to estimate depth. Ren Ng's company, Lytro, commercialized this technology in light field cameras [@ng2005-lightfield]. These cameras allow users to perform novel operations, such as refocusing an image after it has been captured (@sec-sensor-lightfield).

![A light field camera. The schematic shows a pixel array, with each macropixel covered by a pinhole or a small lens (lenslet). Rays incident at different angles pass through the pinhole and are recorded by different subpixels. The camera thus records a ray's position (by the pinhole it passes through) and its angle (by the subpixel it hits). The information collected across the array can be used to estimate the depth of objects in the scene. Figure 5, from @adelson-wang-1992.](images/lightfields/01-lightfields/adelson-wang.png){#fig-adelson-wang .margin-caption width="70%"}
