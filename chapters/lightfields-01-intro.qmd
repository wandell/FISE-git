...
![A sea squirt image from [this nice site](https://goodheartextremescience.wordpress.com/2010/01/27/meet-the-creature-that-eats-its-own-brain/). This one is from Papua New Guinea. Plenty more images (and sea squirts) over there.](images/lightfields/01-lightfields/seasquirt.png){#fig-seasquirt width=50%}
...
![The incident light field is the subset of environmental light field rays that arrive at the image systems’ entrance pupil.](images/lightfields/01-lightfields/01-incident-lightfield.png){#fig-incident-lightfield width="80%" fig-align="center"}
...
![The Stanford Multi-Camera Array (top) comprised 128 cameras that measured the environmental light field from many positions. With this configuration one can recreate the experience of moving side to side as well as back and forth. Other array configurations (bottom) have cameras pointing outward from a point. These capture the light field from many directions and can recreate the experience of looking around the room from the point.](images/lightfields/01-lightfields/01-camera-array.png){#fig-camera-array .margin-caption width="70%"}
...
How do we measure a light field (@eq-lightfield-environment, @eq-lightfield-incident, @eq-lightfield-optical)? Using the ray model of light, the image from a pinhole camera is a measurement of the incident light field at a single location. The intensity at each point in the image measures a small bundle of rays from a single direction. By moving the pinhole or using an array of pinholes, we can sample many locations and accumulate information about the environmental light field.

The camera array in @fig-camera-array measures an incident light field at each camera position, and the array as a whole coarsely samples the environmental light field. These cameras use lenses, not pinholes, which adds a complication: the light at each image point is the sum of rays arriving from across the lens aperture. For the computer graphics application of rendering new views, this integration was not a problem because the goal was to interpolate between the captured images.

In other applications, however, a more precise measurement of the light field is necessary. A particularly important case is characterizing how a lens transforms the incident light field into the optical light field (@eq-lightfield-optical). For example, astronomers measure the light from a distant star to understand how parallel rays are transformed by the atmosphere and their telescope optics. They use this information to design adaptive optics systems that correct for distortions, ensuring that rays from a point source converge to a sharp point in the image. Measuring the optical light field is also valuable in consumer photography, enabling features like post-capture refocusing, as we will see in @sec-sensor-lightfield.

### Shack–Hartmann wavefront sensor {#sec-wavefront-sensor}
Johannes Franz Hartmann suggested a simple and clear way to measure the optical light field. He created a screen comprising an array of pinholes behind the lens. As I observed above, the image behind each pinhole measures the intensity of the rays incident at different angles. Because there is an array of these pinholes, the accumulation of pinhole images measures the rays at multiple angles at multiple positions. By performing this measurement for different wavelengths and polarizations, we sample the optical light field (@eq-lightfield-optical).

Hartmann was a German astronomer and his goal was to look for optical defects in telescope objectives.  His reasoning was based on the ray theory of light, without considering light to be a wave (@hartmann1900-wavefront).

The Raskar version in which you put spots behind the lenslet array and adjust the spots to make the array uniform is kind of funny.  It is intended to measure. 
