...
How do we measure a light field? A pinhole camera provides a simple conceptual answer. An image from a pinhole camera is a measurement of the incident light field at the pinhole's location. The intensity at each image point corresponds to the intensity of a small bundle of rays arriving from a single direction. To learn more about the environmental light field, we can follow Leonardo's lead and move the pinhole to different locations, accumulating information with each measurement.
...
While the camera array in @fig-camera-array does not capture the complete light field, it captures enough information for its intended purpose: rendering a scene from various viewpoints by interpolating between the captured images. The light field concept is a powerful theoretical guide for the design and interpretation of such systems, even if the "true" light field is not measured in its entirety.
...
A second example is measuring the optics of the human eye. Imperfections in a person's cornea and lens can be corrected with spectacles or laser surgery. To design these corrections, we must first measure how the eye's optics transform incoming light rays. These measurements are central to the fields of ophthalmology and vision science, as we will explore further in @sec-human-overview.

Modern imaging systems are increasingly designed to measure aspects of the optical light field directly. Many digital cameras use this information for autofocus (@sec-focus-control). Researchers have also developed specialized cameras that capture more detailed light field information. For example, @adelson-wang-1992 built a camera that measures ray intensity from multiple angles at each sensor location to estimate depth. More recently, commercial light field cameras, pioneered by Ren Ng's company Lytro, were developed to measure the optical light field in detail [@ng2005-lightfield]. These cameras and their associated algorithms allow users to perform novel operations, such as refocusing an image after it has been captured (@sec-sensor-lightfield).

::: {.callout-note title="Adelson and Wang's plenoptic camera"}

Brief summary of what they did.  Maybe an image from their paper.
