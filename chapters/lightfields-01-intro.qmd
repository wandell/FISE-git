...
People in many fields are familiar with the concept of the light field, but they use different terminology. Physicists often describe the environmental light field as the spectral radiance of the environment (see this [lovely description from Feynman](https://youtu.be/FjHJ7FmV0M4?si=wi2twZVYjF9KJGRY){target=_blank}). The general *light field* terminology was introduced into optical engineering by @gershunLightField1939 as part of his work in understanding how to design lighting environments, such as the lighting in school rooms and public places[^lightfields-intro-3]. Ted Adelson and Jim Bergen [-@adelson-plenoptic-1991] used the term *plenoptic function* to describe the same idea[^adelson-plenoptic]. The great value of the light field concept in computer graphics was explained by Marc Levoy and Pat Hanrahan [-@Levoy-1996-light-field]. I will use the light field terminology because I find it more familiar and less intimidating than spectral radiance, electromagnetic radiation, or plenoptic.
...
A large, rectangular camera array to coarsely measure the environmental light field was built by my colleagues at the Stanford Graphics Lab [@Levoy-1996-light-field, @fig-camera-array]. These cameras measure multiple views; all the cameras point in the same direction. The camera aArrays at the bottom of the @fig-camera-array capture multiple directions from a single position. Each of these camera arrays acquire enough information so that we can render images that match what a person would see as they look around. Camera arrays that acquire enough information for both 360 deg and slightly different positions, sometimes called stacked omnistereo, have also been built [@thatte-2017-6dof]. From these data, along with an interpolation strategy, we can calculate images from a range of viewpoints. Later in this section I will describe methods for measuring the light field more precisely (@sec-lightfield-measurement).
