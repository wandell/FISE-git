...
There are several types of measurements that are useful for characterizing human spatial sensitivity.  In the first part of this chapter, I review how the initial encoding within the eye limits spatial resolution, through blurring by the optics and sampling by the cone photoreceptors.  These measurements are only one of the steps in seeing, but they are important because they represent a bottleneck. Spatial information that is not captured at by the eye can only be guessed at by the brain.

For many aspects of seeing, there is a large gap between the information available in the cone photoreceptor array and human performance. We perform at a level far below capacity because of the neural processing strategies used by the brain to interpret the image. This gap is of interest, of course, for image systems engineers who are interested in predicting human perception, such as perceived image quality.

While the limits of resolution can be established from studying the biology, information about the final human performance generally come from direct measures of human performance. Some of these measurements are amenable to quantification and thus use in engineering metrics.  Other measurements that are useful for revealing the principles implemented in the neural processing, but not immediately applicable to image systems evaluation or design.  

The material in this section describes measurements that are relevant the engineering metrics. [Foundations of Vision](https://foundationsofvision.stanford.edu){target="_blank"} includes many more measurements that are helpful for understanding the general processing strategies.
