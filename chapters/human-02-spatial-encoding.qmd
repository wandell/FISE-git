...
![Human vision stages](images/human/02-spatial-encoding/02-encoding-overview-human.png){#fig-human-encoding-overview width="80%"}


Later stages of visual processing present different challenges. Signals enter the eye, but the resulting perceptual experience—a color, shape, or texture—cannot be read out with a simple physical probe. Instead, the visual cortex performs perceptual inferences based on the retinal encoding (@sec-human-seeing). Understanding these perceptual stages requires psychophysical experiments and computational models of inference rather than direct transfer-function measurements.

Vision scientists also investigate the detailed neural circuits throughout cortex and how specific cell types mediate visual experience—topics explored in depth in [Foundations of Vision](https://foundationsofvision.stanford.edu){target="_blank"}. 

In this chapter, our focus is deliberately targeted: we quantify how the front end of the human visual system limits our ability to detect and discriminate spatial patterns in the light field. In the next chapter, we target how the front end of the visual system encodes the wavelength. These analyses are the foundation for the most imortant engineering tools needed to model human visual performance
