...
A simplifying feature of the retina, considered as a unit, is that it is a feed-forward system; there is no direct neural feedback from the brain to the retina. This means we can study the retina's input-output relationship without accounting for top-down signals from other brain regions—a luxury not often afforded when studying most other parts of the brain.

For considering vision as a whole, however, we must recognize that the brain does send signals to the eye.  These control various properties, such as the size of the pupil and the optical power of the lens. Perhaps most importantly, the brain provides a form of feedback by controlling eye, head, and body positions. These signals determine which part of the visual world is projected onto the retina. These signals do not directly reach the retina, but they are of huge importance for seeing.
...
Calculating with the Artal data. Rendering is not good enough yet.

::: {#fig-cone-psf- .figure}
::: {.panel-tabset}
### Fovea
![Mumble.](images/human/02-spatial-encoding/conePSF-0-deg.svg){#fig-mosaic-0 width="70%"}

### 3 deg
![Fix.](images/human/02-spatial-encoding/conePSF-3-deg.svg){#fig-mosaic-3 width="70%"}

### 6 deg
![Fix.](images/human/02-spatial-encoding/conePSF-6-deg.svg){#fig-mosaic-6 width="70%"}

### 12 deg
![Fix.](images/human/02-spatial-encoding/conePSF-12-deg.svg){#fig-mosaic-12 width="70%"}
:::
ISETBio simulations of the PSF superimposed on the cone sampling mosaic.  The four panels show four different eccentricities. The optical blur is from one example subject based on Artal optics measurements. Source: fise_humanConePSFm
:::

### PSF wavelength dependence
In normal viewing, the human eye's optics brings the middle wavelengths are in good focus at the inner segment.  Because of chromatic aberration, the short wavelength light is focused at the RGC layer. Measured at the inner segment layer, the short wavelength light is considerably spread out (@fig-cone-psf-wavelength). This defocus of the short wavelength light (chromatic aberration) is anticipated as part of the design of the cone sampling density.  We can see this by comparing the spacing of the L- and M-cones with the S-cones. There are about 3-5 cones within the point spread of the middle wavealength light; There are 3-5 S-cones within the point spread of the short-wavelength light.  The spatial sampling of each cone type matches the optical blur of the wavelengths they capture.
...
We can simulate this in the central fovea using ISETBio.
...
:::
The two tabs show simulations of the cone excitations in response to a point of light.  The left panel is for a 550 nm light and the right for a 480 nm light. The simulation is for the central fovea region.
:::

## Modulation Transfer function 

TODO:  Examples of the rendering of Gabor wavelets of different frequencies on the cone mosaic.  Different eccentricities.  Different wavelengths.

Relate this to the transform chapter.

Figure out where to put these callouts, and finish them up with some more images.
...
Hopefully these ideas are familiar to you by now - it is the same principle we used when describing the light field camera (@sec-lightfield-measurement). Because there is an array of these pinholes, the accumulation of pinhole images measures the rays at multiple angles at multiple positions. In principle, by performing this measurement for different wavelengths and polarizations, we sample the optical light field (@eq-lightfield-optical).
