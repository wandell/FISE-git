...
Hopefully these ideas are familiar to you by now - it is the same principle used by the light field camera (@sec-lightfield-measurement, @sec-sensor-lightfield, @fig-lightfield-pixel). Because there is an array of these pinholes, the accumulation of pinhole images measures the rays at multiple angles at multiple positions. In principle, by performing this measurement for different wavelengths and polarizations, we sample the optical light field (@eq-lightfield-optical).
