...
Many of the system components we characterized in previous chapters had inputs and outputs.  We studied, say a lens, by understanding how the input light field was transformed into an output light field. The transfer functions are the tools we use to implement simulations of the imaging system.

There are parts of the human visual system where we can take the same approach. For example, the eye has a cornea and lens and we can perform measurements to understand how the physiological optics transforms the incident light field.  But there are other parts of human vision that do not have an output we can measure directly; a signal enters our eyes but the percept we experience - a color, a shape, a texture - is private. The ability to study certain aspects of human perception relies on indirect measurements and theoretical treatments that are not input-output transformations. 

In this book, we focus on the parts of vision that can be quantified and measured, frequently by using transfer functions. In this chapter we review the first steps in visual encoding, how blurring by the optics and sampling by the photoreceptors limits the spatial resolution. These measurements are important because they represent a bottleneck for all of vision. Spatial information that is not captured by the eye can only be guessed at by the brain. Information about the spatial resolution of human performance are bounded by measures of this performance. Many of these measurements are amenable to quantification and they can be used to create quantitative engineering metrics (e.g., Barten's model).  

There are many additional interesting questions that vision scientists pursue.  For example, there is more information available in the cone photoreceptor array than we effectively process. Why does the brain only select some of the information?  This gap is also of practical interest to image systems engineers. The parts we encode but do not perceive enables us to devise much more efficient optical designs. 

Vision scientists are also be interested in the specific neural circuits of the visual system. How are they organized and what do they compute?  This is the material I treat in [Foundations of Vision](https://foundationsofvision.stanford.edu){target="_blank"}, which includes many more measurements that are helpful for understanding the neural processing strategies.

The material in this book is narrowly focused on describing measurements that are relevant to engineering metrics, how well we detect and discriminate different aspects of the light field. I particularly emphasize specific tools that we use to quantify the limit6s of human performance. These practical matters are also interesting, important, and often connected to the general scientific inquiries. 
