...
The idea of shift invariance - whether a shift in space or a shift in time - is a very important special case of a linear system. When a system is shift-invariant, we can model it and experimentally characterize it very efficiently. In optics, the system as a whole is not often shift-invariant.  But, one often finds a region near the main optical axis of a system that is spatially shift invariant. Our colleagues in optics call these *isoplanatic* regions.  

Some notation about isoplantic regions is useful, just as a reminder. Suppose we express image translation as a function. Specifically, we write the translation of an image by an amount, $\delta$, as $T(x,\delta)$.  We would be write the isoplanatic region by noting that the linear transform of a translated image is the translation of the linear response. 

$$L(T(x,\delta)) = T(L(x),\alpha \delta)$$.  

Notice that the size of the shift differs on the input and output by a factor, $\alpha$, which depends on the image magnfication.
