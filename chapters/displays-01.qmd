...
![(a) Passive organic light-emitting diode (OLED) pixel array: electroluminescent light is generated when current is applied to the conductive layers of organic material sandwiched between a cathode and an anode. An active matrix OLED includes a thin film transistor that is placed on top of the anode to control the electrical signal at each pixel. (b) Temporal response. The luminance time course for two frames when pixels are set to 97.5% of peak and then (dashed line) 2.5% of peak. The bottom graph shows the temporal profile in a “flicker-free” mode that rapidly turns on and off the OLED pixels within a single frame. (From Cooper, E.A. et al., J. Vis., 13, 16, 2013.)](images/displays/display-oled.png){#fig-display-oled width="70%"}
...
[^1]:  [https://github.com/isetbio/isetbio](https://github.com/isetbio/isetbio): Tools for modeling image systems engineering in the human visual system front end
