...
## Surface spectral reflectance regularities {#sec-surface-overview}

Four things can happen when electromagnetic radiation interacts with a surface. The radiation arriving at a surface might be (a) reflected, (b) absorbed, (c) pass through, or (d) interact with the material and generate new radiation (fluorescence). We will consider each of these interactions in different sections of this book.  This section describes some regularities about how light of different wavelengths is reflected from many common materials. 

The radiation incident on a small, planar patch of material can come from different directions, which we can specify by two angles. The intensity of the reflected radiation will also vary with direction (two more dimensions). The incident and reflected light can be measured as the intensity for different wavelengths and polarizations (two more dimensions). Hence, a full description of surface spectral reflectance requires specifying multiple parameters [@sec-properties-brdf].

Rather than confronting the full complexity right away, we will consider several relatively simple cases that are useful to understand. We will work our way up from the simplest case to increasingly complex descriptions, identifying regularities along the way[^fluorescence-skipped].

[^fluorescence-skipped]: We will set aside fluorescence for the present, returning to it later.
...
![Comparisons of (a) diffuse, (b) glossy specular, (c) perfect specular, and (d) retroreflective materials. Figure 9.1 from PBR book. https://www.pbr-book.org/4ed/Reflection_Models](images/lightfields/04-spectral/specular-diffuse-retro-revised.png){#fig-specular-diffuse-retro .margin-caption width="80%"}

### Retroreflective materials {#sec-retroreflective-materials}
A fourth interesting case are **retroreflective materials**. For these materials the reflected ray is sent back in the same direction as the incident ray. These materials can be created by embedding small, spherical, glass beads with a metallic back surface into the material @fig-retroreflective-glass-beads. The glass beads refract the incident ray, the metallic surface returns the ray, which is again refracted, so that the net effect is the reflected ray is returned in the direction of the incident ray. Glass beads with a metallic backing are often built into road markings and road signs. Light from a car's headlight is scattered back towards the driver, increasing the visibility of the road marking or sign during nighttime driving.

![Glass beads made into a retroreflective material. (Source: roadmarkingsupplies.com, https://roadmarkingsupplies.com/reflective-glass-beads-wholesale-for-road-marking-paints/)](images/lightfields/04-spectral/01-retroreflective-glass-beads-revised.png){#fig-retroreflective-glass-beads width="80%"}

Retroreflection relies on specific surface properties that enable a material to reflect light back towards its source, regardless of the angle of incidence. The key property for retroreflection is the incorporation of specialized microscopic structures on or within the material's surface. These elements are designed to intercept incoming light and redirect it back along a path parallel to its original direction. The two main types of elements are:

* Glass Beads (Microspheres): These are tiny, transparent spheres embedded in a binder material or coated on the surface. Light enters the bead, refracts (bends), reflects off a mirrored backing on the opposite side, refracts again as it exits, and is directed back towards the light source. The roundness, clarity, and refractive index of the beads are critical for efficient retroreflection.
* Cube Corner Prisms (Microprisms): These are tiny, three-sided pyramid-like structures arranged in a precise pattern on the material's surface. Light enters the prism and undergoes total internal reflection off each of the three mutually perpendicular faces. This series of reflections sends the light back towards the source. The angles and smoothness of these prism faces are critical for performance.

The pattern and the number of retroreflective elements per unit area on the surface influence the overall retroreflective intensity and the range of angles over which the material is effective. A higher density of well-aligned elements generally leads to brighter retroreflection. 

::: {.callout-note title="Natural retroreflection" collapse="false"}
Several natural phenomena and biological structures exhibit retroreflection:

* **Animal Eyeshine:** The most familiar biological example is the glowing eyeshine seen in nocturnal animals such as cats, deer, owls, and crocodilians when illuminated in the dark. This glow is produced by the *tapetum lucidum*, a reflective layer behind the photoreceptors in the retina. The eye acts as a natural cat's eye retroreflector: the cornea and lens focus incoming light onto the tapetum, which reflects the unabsorbed light back out through the same optics, re-collimating the rays back toward the light source. This mechanism gives photoreceptors a second chance to catch photons, boosting sensitivity in dim light. An example in the alligator eye is shown in @fig-alligator-eyeshine.

![In my courses and books on vision science, I often use images taken by William Rushton and Clive Hood in their Florida lab to illustrate the presence of the rhodopsin photopigment in the eye of an alligator [@rushton1975-alligator]. The image on the left shows the unbleached rhodopsin (purple); when the photopigment is bleached away by light (right), the underlying reflective tapetum becomes clearly visible. Light reflecting from the tapetum is returned back toward the source by the eye's optics, producing the bright glow known as *eyeshine*.](images/lightfields/04-spectral/alligator-eyeshine.png){#fig-alligator-eyeshine width="80%" fig-align="center"}

* **The Moon:** The Moon's surface exhibits significant retroreflection (the *opposition surge*). This is largely due to the microscopic structure of the lunar regolith (powdery dust) and shadow-hiding, which scatters light preferentially back toward the direction of incoming sunlight. This is why the full moon appears much brighter than a Lambertian surface would predict.

* **Dew Drops and Spider Silk:** Under early morning conditions, you can see a bright halo around the shadow of your head cast onto dew-covered grass. This phenomenon, called the *Heiligenschein* (holy glow), occurs because spherical dew droplets act as tiny spherical lenses with the grass blades behind them acting as focal reflectors—nature's version of the glass beads described above. Similarly, some spider silks exhibit retroreflection because their cylindrical structure acts as tiny cylindrical lenses.

* **Insects:** Certain insects possess specialized microscopic cuticular structures that produce retroreflection, which can function in signaling or camouflage.

While engineered retroreflectors are manufactured for safety signs and clothing, nature has independently evolved optical structures to direct light back to its source for millions of years.
:::
