...
## The Big Picture {#sec-optics-wavefront-bigpicture}

In my classroom experience, students who have taken Fourier transform courses are often struck by the elegant principle that the point spread function (PSF) of an imaging system can be described directly by the shape of the aperture. It is a neat observation that has the great virtue of being true. In the context of an engineering course, however, this observation can be slippery to integrate because the qualifier *"diffraction-limited"* slips by so easily. I want to take a few paragraphs to build an intuitive, physical picture integrating what we have learned about apertures, lenses, wavefronts, and even pinholes.

First, the wavefront at the exit pupil and the image intensity are mathematically linked. The complex pupil function (@eq-pupil-function) describes both the aperture transmission $a$ and the phase shape of the exiting wavefront. The Fourier transform of this pupil function yields the complex amplitude spread function (@eq-psf-fourier), and its squared magnitude determines the recorded image intensity, which is the PSF (@eq-psf-definition). But there are crucial subtleties in how this relationship behaves, and as my wise colleague Stuart Anstis once advised me, *"Subtlety doesn't work."*  So let's not be subtle.

**First**, the phase term in the pupil function, $W(u, v)$, is defined as the deviation from an ideal spherical wavefront converging precisely onto the sensor plane. Because of this definition, if we change the location of the sensor—moving it away from the conjugate focus ($d_s \neq d_i$)—the reference sphere changes, and a quadratic phase error (defocus) appears in $W(u, v)$. Furthermore, the physical distance to the image plane sets the spatial scale of the diffraction pattern on the sensor ($w \propto \lambda d_i / D$).

**Second**, when an optical system is truly diffraction-limited, the optics introduce zero wavefront errors, meaning $W(u, v) = 0$. In that ideal case, the pupil function reduces strictly to the binary aperture mask, $a(u, v)$. This is where the rule in the Fourier transform course originates: in this case the PSF is determined entirely by the Fourier transform of the aperture opening alone.

**Third**, real optical systems rarely remain diffraction-limited across a wide range of conditions. In general, image blur arises from two competing contributions: edge diffraction set by the aperture geometry $a$, and geometric/optical blur described by phase errors $W$. The relative importance of the two depends heavily on the operating regime:
...
We encountered a version of this tradeoff earlier when analyzing the simple pinhole camera (@fig-pinhole-chessset, @sec-airy-pinhole). For a pinhole of diameter $D$, an extremely small hole blurs the image through edge diffraction, while an oversized hole blurs the image through geometric projection. The exact same principle governs general lens systems: total blur is an ongoing balance between aperture diffraction and wavefront phase errors.
