...
It is useful to write the environmental light field as an explicit function so that we can keep track of its various parameters. For the moment, it is simple and convenient to consider the radiation as comprising a large set of light rays. At each point in the volume of space, $(x,y,z)$, there are rays traveling in many directions. We specify these directions by two angles $(\alpha,\beta)$, the azimuth and elevation. Each ray has wavelength $\lambda$ and polarization $\rho$. Thus, the intensity of the environmental light field function, $L_E$, depends on seven parameters.
