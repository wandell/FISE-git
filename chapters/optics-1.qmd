...
A small pinhole can create a clear image, so why do we need additional optics? The key reason is that pinhole cameras capture only a limited amount of electromagnetic radiation.

Let's clarify what "small" means in this context. Imagine you're in a dimly lit room with a broad wavelength spectrum, providing an illumination of 10 cd/m². If you use a camera with a 1 mm pinhole, a sensor with 1-micron pixels, a focal length of 4 mm, and an exposure time of 50 milliseconds, only about 35 photons will reach each pixel. For color imaging, the situation becomes even more challenging because the light must be divided into different wavelengths. Due to unavoidable photon noise, which we'll discuss later, this limited signal allows reliable intensity discrimination only when there is more than a 20% difference in brightness between two pixels.

Lenses offer a solution by enlarging the entrance aperture to capture more photons. They can use larger apertures to gather more light without increasing image blur. This ability is likely why lenses have evolved across many species.
