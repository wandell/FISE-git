...
## Light Field Cameras {#sec-sensor-lightfield-camera}
A modern image sensor design that directly captures the optical light field was invented by @adelson-wang-1992. Using the same physical arrangement as the Shack-Hartmann wavefront sensor (@sec-shack-hartmann), they placed a lenslet array in front of a standard camera sensor. In this context, the combination of each lenslet and its underlying group of sensor pixels is often called a "macropixel," with the individual detector elements beneath referred to as "subpixels." As @fig-lightfield-pixel illustrates—using a pinhole array rather than a lenslet for simplicity—rays arriving from different angles are captured by different pixels behind the pinhole. This sensor records both the position of a ray (which lenslet or pinhole) and its angle (which subpixel). @adelson-wang-1992 describe how to use this information to estimate depth.

![A light field sensor captures both the position and angle of incoming light rays. The sensor is composed of an array of "macropixels" (bottom). Each macropixel consists of a microlens (or a pinhole, as simplified here) and a group of pixels beneath it. The microlens or pinhole location on the sensor determines the ray's position. The subpixels are below the pinhole. Which of these detects the light determines the ray's angle. By capturing the position and angle data across the entire sensor, a light field camera can perform powerful computations, such as refocusing an image after it has been taken. (Image by me and Gemini 2.5 Pro.)](images/sensors/19-innovations/Gemini_lightfieldPixel-Circle.png){#fig-lightfield-pixel width="50%"}

The idea was further developed by Ren Ng and his colleagues. Ng formed a company, Lytro, that commercialized light field cameras [@ngLightFieldPhotography2005]. These cameras allow users to perform novel operations, such as refocusing an image after it has been captured.

Full light field cameras, particularly used for depth estimation, are not common in consumer devices. But many cameras use a simplified version of this technology for autofocus (@sec-focus-control). The dual-pixel autofocus (@sec-dualpixel-autofocus) samples just two angular positions per macropixel, rather than the many angles captured by a full lenslet array. 
