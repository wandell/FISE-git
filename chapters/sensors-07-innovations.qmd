...
The idea was further developed by Ren Ng and his colleagues. Ng formed a company, Lytro, that commercialized light field cameras [@ng2005-lightfield]. These cameras allow users to perform novel operations, such as refocusing an image after it has been captured.

The principles of the light field camera continue to be used in modern sensors. While full depth estimation is not common in consumer devices, many cameras use a simplified version of this technology for autofocus (@sec-focus-control): dual-pixel autofocus (@sec-dualpixel-autofocus) samples just two angular positions per macropixel, rather than the many angles captured by a full lenslet array. The lenslet array itself reappears in a very different application—measuring rather than capturing the light field—in the Shack-Hartmann wavefront sensor (@sec-wavefront-sensor), used to characterize telescope and eye optics.
