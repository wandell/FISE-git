...
Ray tracing is a framing that feels very physical.  We can imagine the reality of rays and the way their path is controlled by passing through a sequence of lenses, or reflected by mirrors, or scattered by materials. We are headed to a second, somewhat different, way of thinking about optics. This framing treats optics as a function that takes an input and produces an output. The 

For each optical element, you can describe its effect on a ray of light using a **ray transfer matrix**. The main matrices used are:
