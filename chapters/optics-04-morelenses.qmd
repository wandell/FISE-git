...
The important point is this.  I won't show the derivation, but it turns out that we can calculate the new angle, $\theta_2$ as $\Phi + \theta_1$.  The important observation, that makes this whole thing work, is that $\Phi$ doesn't depend on $\theta_1$.  We can calculate $\Phi$ from the indices of refraction and curvatures.

$$
\Phi = \frac{n_{2}-n_{1}}{n_{1}} \left(\frac{1}{R_{1}} - \frac{1}{R_{2}}\right)
$$

That means we can express the transformation of the ray $(y_1, \theta_1)$ into $(y_2, \theta_2)$ as 2x2 matrix,
...
Very cool.  And for a thin lens, even more cool, it turns out that
...
so the matrix can also be written this way
...
of light as it (a) crosses a surface, or (b) continues through the medium, using a. The whole journey, from the object side to the image side, is a sequence of such transforms and can be described by mutiplying the matrices together. 
