...
The transition from film to digital imaging, enabled by semiconductor technology, has profoundly changed how we record and process images (@boyle1970-ccd-first; see [Preface](../index.qmd#sec-preface)). The initial digital sensors used arrays of tiny, light-sensitive elements called **photodiodes**—semiconductor devices that convert incoming light into electrical current. When photons strike the photodiode’s active region, they generate electron-hole pairs, resulting in a measurable flow of electrons. Each photodiode forms the basis of a pixel in modern image sensors.

The earliest devices, known as charge-coupled devices (CCD), were built using metal-oxide semiconductor technology (MOS). These sensors required significant power, making them unsuitable for mobile devices and limiting their early adoption to scientific and high-end applications.

A major breakthrough came with Eric Fossum's invention of the Complementary Metal-Oxide Semiconductor (CMOS) image sensor (@Fossum1993-ccd-dinosaurs; @Fossum1995-patent). By implementing an active pixel sensor (APS) circuit, Fossum enabled image sensors to be manufactured using mainstream CMOS microelectronics processes. This dramatically reduced power consumption and allowed integration of light-sensing pixels with timing and signal processing circuitry on the same chip (@fig-olympus-overview). These advances paved the way for the widespread adoption of electronic image sensors in mobile devices and many other applications.
...
Photodiodes are the essential building blocks of all modern image sensors, both CCD and CMOS. The physical principle underlying the operation of the photodiode—converting optical images into electrons—is called the **photoelectric effect**. Understanding how materials convert light into electrical signals is a fascinating chapter in the history of physics and engineering, shaped by groundbreaking experiments and theories. We begin this section on sensors by describing the discovery of this phenomenon and its fundamental properties.
