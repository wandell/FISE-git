...
The energy from electromagnetic radiation generates an electron in a semiconductor by creating an electron-hole pair. The bandgap gets explained here, with some numbers. 

![Photon-generated electrons and the bandgap](images/sensors/03-sensor-bandgap.png){#fig-sensor-bandgap width=50%}

Silicon crystal are a core technology for electronics. Silicon crystals are used for processors and memory and nearly all ‘chips’. Atoms in a  silicon crystal wafer contain electrons in discrete bands called the Valence Band (lower energy) and the Conduction Band (higher energy). Most electrons occupy the Valence band. 

The absorption of a photon by the silicon can excite electrons into the Conduction band. The excitation energy in the photon must be at least 1.14 electron volts. When the electron moves to the Conduction band, it leaves a ‘hole’ in the valence band which acts like a positively charged carrier. For the electron to change bands requires adequate energy. A 1100nm photon of light has 1.14eV, which is the minimal amount of energy needed to move an electron between the the two bands in silicon.  This is called the **band gap**. Silicon imagers are insensitive to electromagnetic readiation beyond this wavelength and other materials are needed.

Ordinarily, the hole and electron will re-combine and be lost.  To make an imager, we need to measure the number of electrons. In a CCD an electric field between the bands prevents recombination. In CMOS electrons are trapped in capacitors placed in the silicon. We explain how this is done in @sec-p.
