...
Isaacson also details how the experimental physicist Lenard, whose work was important to Einstein's theory and who was also a Nobel Laureate, became an outspoken anti-Semite. Lenard actively opposed Einstein’s recognition in the scientific community. As Isaacson also chronicles, Bohr and Einstein maintained a deep mutual respect and professional friendship throughout their careers.
...
## Electrons in semiconductors: the bandgap
Atoms in a silicon crystal are arranged so that their electrons occupy specific energy levels grouped into two main bands: the **valence band** (lower energy) and the **conduction band** (higher energy). Under normal conditions, most electrons remain in the valence band. When a silicon atom absorbs a photon with enough energy, an electron can be excited from the valence band to the conduction band (@fig-sensor-bandgap). The minimum energy required for this transition is called the **bandgap**, measured in electron volts (eV). For silicon at room temperature, the bandgap is 1.12 eV. Only photons with energy equal to or greater than this value can generate free electrons in silicon.

![Energy bands in silicon. A photon with energy greater than the bandgap (1.12 eV) can excite an electron from the valence band to the conduction band, generating a free electron.](images/sensors/03-sensor-bandgap.png){#fig-sensor-bandgap width=50%}

Without special circuitry, the hole and electron will re-combine. To make an image sensor, we need a method to measure the number of electrons. In a CCD an electric field between the bands prevents recombination. In CMOS electrons are trapped in capacitors placed in the silicon. We explain how this is done in @sec-sec-pixels.

There are several wavelength-dependent effects worth noting about silicon sensors. First, a photon with a wavelength of 1100 nm has just enough energy (1.12 eV) to move an electron between the two bands in silicon. Thus, silicon sensors are insensitive to electromagnetic radiation beyond 1100 nm, and to create an infrared sensor we must use a different material. Second, wavelengths shorter than 1100 nm all have enough energy to excite an electron across the bandgap, so silicon sensors will absorb radiation at these shorter wavelengths, which the eye does not encode. Finally, different wavelengths of radiation are typically absorbed at different depths within the silicon. Higher-energy, short-wavelength photons are absorbed and generate electrons near the surface; lower-energy, long-wavelength photons are absorbed deeper in the material. This depth-dependence on wavelength has been used to create color image sensors, as described in @sec-sensors-foveon.
