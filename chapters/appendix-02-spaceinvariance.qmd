# Linear Space-Invariant (LSI) Systems {#sec-appendix-shiftinvariance}
...
We have reached the observation that is the basis of many calculations in the main chapters in the book: the relationship between harmonics and space invariant linear systems. These ideas are introduced in the main text in @sec-optics-linear-harmonics, where we develop the Discrete Fourier Series (@sec-optics-fourier-series). The ideas continue to be used in many subsequent chapter.
...
The fact that harmonics are eigenvectors of space-invariant linear (LSI) systems leads to one of the most powerful results in signal processing: the **Convolution Theorem**.
