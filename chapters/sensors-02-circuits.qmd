...
To create an image, the sensor counts the number of electrons generated at each photodiode during the exposure. Each photodiode is connected to a small capacitor that collects and stores these electrons. When it is time to read out the image, specialized circuitry measures transfers the charge stored in each capacitor to an analog-to-digital converter (ADC), which converts the voltage of the stored electrons into a digital value.  This digital value is roughly proportional to the number of photons incident at the photodiode. After the measurement is complete the circuitry resets the capacitor, clearing any remaining electrons to prepare for the next exposure.
