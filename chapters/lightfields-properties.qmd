...
Suppose that the incident light comes from a direction represented by the unit vector $\mathbf{i}$, and the surface normal vector is $\mathbf{n}$. The direction of the reflected ray, $\mathbf{r}$ is calculated this way:

$$
\mathbf{r} = \mathbf{i} − 2 (\mathbf{i} \cdot \mathbf{n}) \mathbf{n}
$$  {#eq-mirror}

The term ($\mathbf{i} \cdot \mathbf{n}$) is the projection of the incident ray onto the normal direction. We subtract twice that amount from the incident ray, which flips this component of the incident ray. That is, the reflected ray has the same component length in the normal as the incident ray, but its direction is reverse. It has exactly the same vector component in the surface plane. The light reflected at the interface does not interact with the molecules in the material, and thus its spectral power distribution is the approximately equal to the light source.
...
Common mirrors are very to perfect specular reflectors in the sense that a very small set of collimated rays is reflected over a very small angle of output rays. For scientific applications it is possible to polish mirrors, making them increasingly perfect. Typically, however, materials are approximately specular, but not perfectly so. In these materials a small group of incident rays is reflected mainly in the specular direction, but there is some spread of the output rays compared to the spread in the incident rays.  @fig-specular-diffuse-retro compares different conditions, including a diffuse reflectance, nearly perfect specular, and mostly specular case.

![Comparisons of specular, diffuse and retroreflective materials. Figure 9.1 from PBR book. https://www.pbr-book.org/4ed/Reflection_Models](images/lightfields/specular-diffuse-retro.png){#fig-specular-diffuse-retro}

There is fourth case that is quite interesting and used in practical applications:  retroreflective.  Some materials have the property that the incident ray is sent back in the same direction as the incident ray - either quite precisely or at least approximately.  

https://docs.google.com/document/d/1BtxldFF14EY4eC9x6KI9cy1RF4GJAy-e7UAgdJ5YsfI/edit?usp=sharing

<!--
Good start for DRM and mirror and diffuse reflectance.  
https://commons.wikimedia.org/wiki/File:Specular_v_Diffuse_reflection.png

Also, note the specular-diffuse-retro.png file.  From the Korean kid's blog and also PBR book.
...
The dichromatic reflectance model (DRM) is another simplified but useful surface reflectance model. It is a good approximation for light reflected from **inhomogeneous dielectric materials** (non-conductors like plastics, paints, wood, skin, etc.). The model approximates the material reflectance as a weighted sum of a diffuse term and a specular (mirror) reflectance. This model is used in computer vision applications where the user would like to identify specular reflections.

A portion of the incident light is reflected at the surface (**interface reflectance**).  This light has a mirror-like reflectance that preserves the spectral power distribution. Another portion of the light enters into the material (**subsurface scattering**) where it interacts with the material's pigments, both by scattering and absorption. This portion of the incident light is a diffuse reflectance. The subsurface reflectance is usually assumed to be Lambertian, as the subsurface pigments are randomly positioned and oriented. The DRM represents the reflected light, $C(\lambda)$ as the sum of the interface and subsurface terms
