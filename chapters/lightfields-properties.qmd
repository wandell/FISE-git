...
Linear models appear in many places in this book, with many different applications. When you are ready to read more about them as a general mathematical tool, go to the Appendix chapter on linear models. It has a chatty discussion that can help you find your way into the more advanced literature.
...
Over the decades a number of imaging groups have measured the reflectance of many types of materials, including paints, minerals, plants, clothing, and various biological specimen such as skin and teeth. Government agencies, particularly those involved in remote sensing, have measured the reflectance of many materials that might be seen in satellite images. As a general rule, the spectral reflectances of the samples reported in the literature are approximated to an accuracy of a few percent by linear models using between 3 and 8 basis functions. A Finnish group, for example, measured 1269 different color swatches from the Munsell Book of Colors. This widely used book spans a wide range of color samples that provides designers with a method for identifying, selecting, matching, and communicating about color. @Parkkinen1989-munsellspectra made their spectral measurements at 1 nm resolution. A linear model using three basis functions

![Three basis functions for the Munsell Book of Color, from measurements by @Parkkinen1989-munsellspectra. The linear model with these bases approximates the measurements with a root mean squared error of 2.5%. Using five basis functions, the RMSE is about 1%.](images/lightfields/01-munsell-basis.png){#fig-munsell-basis fig-align="center" width="60%"}

The best linear models for different types of common samples are quite similar. An early set of measurements Mike Vrhel and colleagues @Vrhel1994-MeasurementAnalysisObject included dozens of surfaces from each of several different groups (paint, natural objects, man-made objects, food). The best linear models from each of these categories were quite similar to one another and to the Munsell Book of colors.

![Three basis functions comparing the basis functions derived from the @Parkkinen1989-munsellspectra data (upper left) and different categories measured by @Vrhel1994-MeasurementAnalysisObject.](images/lightfields/01-reflectance-bases.png){#fig-reflectance-bases fig-align="center" width="60%"}
...
Maybe we need a bit about dichromatic reflectance model here to open up.  Steven Shaefer.

### Bidrectional reflectance distribution function (Data driven)
...
@MatsuMatusik2003-datadrivenBRDF
...
### Bidrectional reflectance distribution function (Theory)


```{=html}
<!--

Sand, reasonable figures.Tongbuasirilai2020-BRDFTongbuasirilai2020-BRDFTongbuasirilai2020-BRDFTongbuasirilai2020-BRDFTongbuasirilai2020-BRDF - https://opg.optica.org/ao/fulltext.cfm?uri=ao-54-31-F243&id=330097
