...
The physical idea is that light is reflected by two paths. Some light is reflected at the material's **interface** with air, without engaging with the material itself. The spectral power distribution of this light is the same as the light source. A second term describes light where the wavefront enters into the material (**subsurface**) interacts with the material's pigments or colorants through scattering and absorption, and then emerges from the surface. This component is responsible for the object's inherent color. Its spectral composition is modified by the material's properties and is generally considered to be Lambertian, meaning it reflects equally in all directions. 

$$
C(\lambda) = m_s(g) I(\lambda) E(\lambda) + m_d(g) D(\lambda) E(\lambda)
$$

Where, $E(\lambda)$, is the spectral power distribution of the incident light, $I(\lambda)$ is the spectral reflectance of the interface reflection (often assumed to be constant across wavelengths for dielectrics. The term $D(\lambda)$ is the diffuse reflectance of the body, which is characteristic of the material. The terms $m_s(g)$ and $m_d(g)$ are geometric scaling factors. Their values depend on the viewing and illumination angles with respect to the surface normal at the point $g$. The total reflection, $C(\lambda)$ is sometimes called the *color signal* because this is the spectral radiance that drives the visual system's color response.

The DRM is more complex than the diffuse model and it finds application in topics such as illuminant estimation, specular highlight identification and removal, and material classification. It is, however, limited in various ways because it does not accurately model complex surfaces, and it does not apply well to scenes with multiple light sources or very large (sky map) light sources.
...
@MatsuMatusik2003-datadrivenBRDF @Tongbuasirilai2020-BRDF
