...
Most commonly the basis functions $B_i$ are chosen to be orthogonal to one another. For example, this is the choice when people use the principal components method. Basis vector orthogonality means that the inner product[^1] of any pair of basis vectors is zero. The formula for the inner product, in this example is,

[^1]: Widely used synonyms for the term inner product are *dot product* and *scalar product*.
...
## Correlated weights
![The daylight basis weights from the Granada and Stanford data sets.  As a gif.  ](images/lightfields/01-dayweights.gif){fig-align="center" width="372"}
