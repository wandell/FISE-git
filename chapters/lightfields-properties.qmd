...
The spectral reflectance function measures the fraction of light incident at the surface that is reflected. For a diffusely reflecting (Lambertian) surface, incident light is reflected in all directions. Tbese surfaces typically have rough microscopic facets, oriented in many directions, which is why the incident light is reflected in many different directions.

The spectral reflectance is measured wavelength-by-wavelength, that is they are functions of wavelength. In the visible band, these functions are typically quite smooth, and it is possible to summarize them using only a few parameters @fig-reflectance-cloths. It is not realistic to obtain a sample of all surfaces, but we might consider the surfaces in a particular context (e.g, walking in a forest, the interior of a commercial office building, driving on a highway, or medical images of the body). The likely surfaces in some context can be sampled and usefully summarized with a linear model [@Cohen1964-linearmodel, @Maloney1986-linearmodels-surface, @Hardeberg2002-reflectancedimensionalty]. The best basis functions will depend, somewhat, on the context; and, the number of basis functions needed will depend on the task requirements. But the general usefulness of a linear model approximation as a computational tool is not in doubt.
...
Over the years a number of groups have measured the reflectance of many types of materials, including paints, minerals, plants, clothing, and various biological specimen such as skin and teeth. As a general rule, the spectral reflectances are approximated to an accuracy of a fraction of a percent by linear models using between 3 and 8 basis functions. There are also many similarities between the best linear models for the different types of common samples. One important early set of measurements came from Mike Vrhel and colleagues @Vrhel1994-MeasurementAnalysisObject, who sampled surface spectral reflectance functions of a few hundred objects of different types (paint chips, natural and man-made objects, food).
