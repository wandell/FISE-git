...
How to specify regularities in the scene, and how to take advantage of them, will be a recurring theme as we describe various image systems. There are two classical mathematical tools that are widely used to specify regularities throughout science and engineering, and both will come up in this book. I just mentioned Bayes' Rule, which is famous and simple.  A second mathematical tool, dimensionality reduction using linear models or techniques from compression, is also very widely used.  

More recently, training neural networks has proven to be a very useful way to infer the statistical regularities of a large training set. Here, I describe the classical techniques; I will introduce some neural network training applications in later chapters.
...
There are a large number of tools to find the basis vectors, $B_i$ .  Sometimes, they arise naturally from physical principles.  This is the case for the Fourier basis functions, which comprise the harmonic functions at different frequencies.  It is common to select $N$ basis functions that explain the most variance in the data. This is the choice we make when using Principal Components Analysis (PCA). In both of these cases, Fourier and PCA, the basis vectors are orthogonal.  This means that the inner product[^lightfields-properties-1] of any pair of basis vectors is zero. The formula for the inner product, in this example is,
