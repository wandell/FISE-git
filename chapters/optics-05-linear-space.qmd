...
> The SELF pathfinder telescope illustrated in Fig. 19 is now under construction and will provide confidence that this is feasible. We believe, as we argued here, that there are no fundamental  reasons  why  we  cannot  build  50  m-scale telescopes  on  the  ground  that  can  reach  sensitivity  levels sufficient to find evidence of life on nearby exoplanets. (@kuhn2025-astronomy).
...
could  make  images  of  the  surface  of  nearby  exoplanets using  data  inversion  techniques,  perhaps  sufficient  to  see even signs of advanced exolife civilizations6,7.

The light reflected from a planet is 10^10 smaller for earth like situation, or for a red star maybe 10^8 or 10^9 times brighter.

The design of the optics needed to see a planet circling a star requires no spread from the star onto the light from the planet.  The angle might be 1 sec of arc between them.  So the point spread must be less down by more than 10^8 or 10^9 within 1 sec of arc.  Otherwise the light from the star will mask the light from the planet.
...
He is an astronomer at the University of Hawaiʻi's Institute for Astronomy and has a long history of working on instruments and technologies for both solar observation and exoplanet detection.
