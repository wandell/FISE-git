...
Linearity is the foundation of how we model the optical transformation of an input scene into a sensor image. We imagine the scene as a collection of points, $\mathbf{p} = (p_x, p_y, p_z)$, each with its own intensity $I(\mathbf{p})$. The optics spreads out the light from each point in a certain way—this is called the point spread function, or $PSF_\mathbf{p}(x, y)$, where $(x, y)$ are positions in the image.
...
The point spread function is an intuitive way to understand the performance of the optical system. But, it is easy to use as for quantifying image quality. The point spread function can be quantified with a few parameters for some cases (Airy pattern, @sec-pointspread-common). But in general we do not have high quality approximations for many real lenses, multielement lenses, or measurements of the human eye.  The following sections describes other methods, also based on linearity, that we use to characterize and understand system performance.
...
Linear models are important for image systems and science and engineering broadly. Of the many linear models, having the bases be **harmonic functions** has been the most important. We are heading that direction, and the way will get there is through eigenfunctions.
