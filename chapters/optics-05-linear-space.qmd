...
It is natural to think of images as a set of points, and it is somewhat disappointing to realize that this representation isn't always helpful for system characterization.  Historically, it was an important insight to realize that images can be described in many other ways. Some of these alternatives are very valuable for analyzing and characterizing optical performance of image systems.
...
When we represent an image as a set of points, the basis functions are points.  That is, $B_i$ is zero everywhere except except at one $(x,y)$ point.  The basis weights are the image intensity at that point, $I(x,y)$.
...
Linear models are important for image systems and science and engineering broadly. Of the many linear models, the **harmonic functions basis** has been the most important. We are heading that direction, and the way will get there is through eigenfunctions.
