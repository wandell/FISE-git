...
There is no guarantee that the point spread is the same everywhere.  In general, it will not be.  This is the case of a **shift-varying** linear system.  We can still represent the system by a matrix, just not a circulant matrix.

If the system is linear, we can already do a lot with a simple calibration to find the system matrix.  

**slidesImageFormationLinearity.mlx**

### Shift invariant linear systems

When the point spread is the same over a region (isoplanatic), we have additional tools.

In that case the system matrix is circulant (shift invariant). Refer to the convolution and shift-invariant parts of the appendix.

Many image systems have locally shift invariant regions.  So, the emphasis on the point spread and shift invariance is central to the field.
...
![Lyot coronagraph concept.](images/optics/optics-lyot-coronagraph.png){#fig-optics-lyot width="80%"}
...
This second example of PSF engineering moves us from the vast scale of astronomy to the tiny world of microscopy.

Biologists often use fluorescent markers to tag specific molecules in tissue samples. These fluorescent tags can be attached to particular cells or molecules, and when excited by light at one wavelength (usually a short wavelength), they emit light at another (usually longer) wavelength. The emitted photons make the tags visible under a microscope. Because a single molecule is extremely small, it acts as a point source, and its image through the microscope optics is a point spread function (PSF).

In many biological experiments, researchers are interested in finding the $(x, y)$ positions and the number of these tagged molecules. If the tissue is sparsely labeled, each molecule produces a PSF that is well separated from the others. To locate and count the molecules, one can simply detect the PSFs in the image. The $(x, y)$ position of each molecule is estimated by finding the centroid of its PSF. Although there is some uncertainty in this estimate due to photon noise and imperfections in the optics, the centroid can be determined with much higher precision than the width of the PSF itself. This approach is called Single Molecule Localization Microscopy (**SMLM**). SMLM techniques are sometimes said to “break the diffraction limit,” meaning they can localize molecules at a finer scale than the size of the Airy disk. While this sounds magical, it is simply a clever use of statistical estimation and optics.

![Double-helix point spread. Using a spatial light modulator to control the phase of the light, Piestun and colleagues created an oriented point spread function (images). The angle of the point spread depends on the distance from the optics at a scale of hundreds of nanometers (graph). After @backlund2012-psfengineering [Figure 1].](images/optics/piestun-double-helix.png){#fig-optics-double-helix width=70%}

When the biological sample has some thickness, it is often important to know the depth ($z$ position) of each molecule, in addition to its $(x, y)$ location. Rafael Piestun and collaborators realized that the PSF of a point source changes with depth, especially in the near field. Normally, this change is just a slight increase in blur size, which is hard to measure accurately. However, Piestun’s group engineered an optical system that produces a PSF with a distinctive shape: two lobes that rotate around a central point as the depth ($z$) changes (@pavani2008-3d-dh-tracking). If you plot the PSF as a function of $z$, the pattern traces out a double helix (@fig-optics-double-helix). In practice, this means that for sparsely labeled samples, you can measure the centroid of the PSF to get $(x, y)$ and use the orientation of the lobes to estimate $z$. This clever PSF engineering enables precise 3D localization of single molecules. Piestun co-founded [Double Helix Optics](https://www.doublehelixoptics.com/) to commercialize this technology for 3D imaging and sensing.

::: {.callout-note title="Single Molecule Localization Microscopy" collapse="false"}
Single Molecule Localization Microscopy (SMLM) is a family of techniques that allow researchers to pinpoint the positions of individual fluorescent molecules with remarkable precision—much finer than the size of the diffraction-limited blur circle of optical microscopy. The Airy Disk size in conventional light microscopy is on the order of a micron. Because SMLM can reveal the position of structures at the nanometer scale, some people refer to these methods as *nanoscopy*.

The origins of SMLM trace back to W.E. Moerner, who first demonstrated that it was possible to detect light from a single fluorescent molecule. He and others later showed that commonly used biological fluorophores could be switched on and off (@moerner1989-singlemolecule, @dickson1997-gfp-moerner). This discovery paved the way for new imaging techniques.

Eric Betzig developed Photoactivated Localization Microscopy (PALM, @betzig2006-PALM), which uses light to activate only a sparse subset of fluorophores at a time. By repeating this process and localizing each molecule’s position with high precision, a detailed map of the sample can be built up over many cycles. Independently, Xiaowei Zhuang introduced a similar approach called Stochastic Optical Reconstruction Microscopy (STORM, @zhuang2006-STORM), which also relies on switching fluorophores on and off to achieve super-resolution.

These advances revolutionized biological imaging. Moerner, Betzig, and Stefan Hell[^hell-sted] were awarded the Nobel Prize in Chemistry for their pioneering work. Xiaowei Zhuang has also received many honors for her contributions to the field. Images from her papers, demonstrating the power of these techniques, are available on [her lab website](https://zhuang.harvard.edu/storm_images.html){target=_blank}.

[^hell-sted]: Stefan Hell developed a different approach, called STED microscopy, which uses a special pattern of light excitation to shrink the region where fluorescence occurs, also achieving very high spatial resolution.
:::
