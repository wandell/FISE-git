...
Do remember that a system can be homogeneous without being linear. For example, $f(x) = |x|$ and the vector length function
...
Be aware of these special cases can be of use to you in the future.

## Point spread functions
Linearity is the foundation that we use to modelg how optical elements convert an input scene into a sensor image. We imagine the scene as a collection of points, $\mathbf{p} = (p_x, p_y, p_z)$, each with its own intensity $I(\mathbf{p})$. The optics spreads out the light from each point in a certain way—this is called the point spread function, or $PSF_\mathbf{p}(x, y)$, where $(x, y)$ are positions in the image.
...
The point spread function is an intuitive way to understand the performance of the optical system. But, it is not a great general tool to quantify image quality. We can quantify the shape of the point spread function for the simplest cases (Airy pattern). And below I describe some point spread shapes that can be described using a small number of parameters and are useful for simulation (@sec-pointspread-common). But the point spread representation alone is not helpful enough to approximate many real lenses, multielement lenses, or measurements of the human eye.  In the following we build up some tools so that we can use other representations that will be very helpful in characterizing and understanding system performance.

## Discrete linear systems {#sec-linear-discrete}
Many of the practical computations we describe in the following sections use **discrete** representations of linear systems. We have already seen discretization when we represented a spatial stimulus at sample points or a spectral stimulus at sample wavelengths.

In these discrete representations, a continuous stimulus, say $s(x)$, is sampled to form a vector of values, $\mathbf{s}$. We often write the entries as $s[x_i]$, where $i$ is an integer index, or simply $s[u]$, with square brackets to remind us that the signal is sampled.

A discrete linear system is then described by a matrix, $\mathbf{L}$, which maps the input vector to an output vector, $\mathbf{o}$:

$$
\mathbf{o} = \mathbf{L} \mathbf{s}
$$ {#eq-linear-matrix}

It can be helpful to visualize this as a matrix tableau: each column of $\mathbf{L}$ is scaled by the corresponding entry in $\mathbf{s}$, and the output is the sum of these weighted columns.

**PUT IN A MATRIX TABLEAU IMAGE HERE**

All discrete linear systems can be represented this way. Sometimes, the input is naturally an image (a matrix), and we need to "flatten" it into a vector; other times, we might use more clever tricks to keep the image structure. But the key idea is that, in the discrete world, linear systems are just matrices that map input vectors to output vectors.

::: {.callout-note collapse="true" title='History - linear algebra and matrices'}

It was an honor for me, as an assistant professor, to attend faculty meetings with Joe Goodman, Ron Bracewell, Tom Cover and others. As we discussed the engineering curriculum, there was often a dispute. My heroes in the older faculty preferred to teach using integrals and continuous functions.  The younger faculty spent most of their working days with computers and programming, and we preferred to teach using matrices and discrete functions. I still remember Ron Bracewell saying something like, continuous functions are the reality and we must explain that first.  After the students understand that, we can introduce them to the vulgar discrete approximations which are important because of computers.  Good times.

Despite these disagreements, we all recognized that it is very useful - and these days essential! - to understand linear systems in terms of vectors and matrices. This has become the most common way to discuss many mathematical methods, and mathematics framed in terms of vectors and matrices is fundamental to the massive wave of neural network calculations in science and engineering.  Where did this representation come from?  In fact, where did the whole framing in terms of linear algebra of vector spaces come from? Click the link.

[History of matrices and linear algebra](resources/history-linear-algebra.html){target=_blank}

The original idea of representing sets of linear equations in terms of matrices - Cayley.

:::

## Spatial basis functions {#sec-optics-spatial-basis}

It is natural to think of images as a set of points; historically, it was an important insight to realize that an image can be described in many other ways. Some of these alternatives are very valuable for analyzing and characterizing optical performance of image systems.

Linear models are an important method for exploring alternative image representations. We introduced linear models in @sec-linear-models, and we applied them to characterizing the spectral properties of light fields. Here we use linear models again, but in a different way. In this case, we build linear models of the image intensity spatial distribution, $I(x,y)$. 
...
An important and somewhat remarkable fact about the collection of eigenfunctions of a linear system is this:  they are an orthogonal basis of the possible inputs.  Any input, $\mathbf{s}$, can be described uniquely as the weighted sum of the eigenfunctions,

$$
\mathbf{s} = \sum_i \sigma_i \mathbf{e}_i .
$$

This means if we represent the stimulus with respect to the eigenfunctions, and we know the eigenvalues, we can predict the output to the stimulus.

$$
\mathbf{o} = L ( s_i \mathbf{e}_i ) = \alpha_i \sigma_i \mathbf{e}_i
$$
